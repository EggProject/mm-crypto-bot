...
class RecordingLogger implements Logger {
  ...
  public debug(message: string, meta?: Readonly<Record<string, unknown>>): void { this.entries.push({ level: "debug", message, meta }); }
  public info(message: string, meta?: Readonly<Record<string, unknown>>): void { this.entries.push({ level: "info", message, meta }); }
  public warn(message: string, meta?: Readonly<Record<string, unknown>>): void { this.entries.push({ level: "warn", message, meta }); }
  public error(message: string, meta?: Readonly<Record<string, unknown>>): void { this.entries.push({ level: "error", message, meta }); }
}
...
class CleanupFailureFeed extends MockExchangeFeed {
  ...
  public async subscribeOrderUpdates(): Promise<SubscriptionId> { return this.nextPrivateId++ as SubscriptionId; }
  public async subscribeExecutions(): Promise<SubscriptionId> { return this.nextPrivateId++ as SubscriptionId; }
...
describe("Bot public runtime boundaries", () => {
  ...
  it("constructs unauthenticated paper market data with configured endpoints", async () => {
    ...
    const bot = new Bot({ config, feed: undefined, exchangeFeedFactory: (options) => { calls.push(options); return feed; } });
  ...
  it("constructs authenticated paper data without optional endpoint overrides", async () => {
    ...
    const calls: Parameters<NonNullable<BotOptions["exchangeFeedFactory"]>>[0][] = [];
    const config = { ...configFor(stateFile), exchange: { ...configFor(stateFile).exchange, id: "bybiteu" as const } };
    const bot = new Bot({ config, exchangeFeedFactory: (options) => { calls.push(options); return feed; } });
    await startThenStop(bot, feed);
  ...
  it("preserves complementary endpoint options across unauthenticated and authenticated factory calls", async () => {
    ...
    const publicConfig = { ...configFor(stateFile), exchange: { ...configFor(stateFile).exchange, id: "bybiteu" as const } };
    await startThenStop(new Bot({
      config: publicConfig,
      exchangeFeedFactory: (options) => { calls.push(options); return publicFeed; },
    }), publicFeed);

    process.env["BYBIT_API_KEY"] = "public-boundary-key";
    process.env["BYBIT_API_SECRET"] = "public-boundary-secret";
    const authenticatedFeed = new MockExchangeFeed({ balances: [{ currency: "USDC", free: 1_000, total: 1_000 }] });
    ...
    await startThenStop(new Bot({
      ...
      exchangeFeedFactory: (options) => { calls.push(options); return authenticatedFeed; },
    }), authenticatedFeed);
  ...
  it("continues after Error and string OHLCV subscription failures", async () => {
    ...
    const warnings = logger.entries.filter((entry) => entry.message.startsWith("[bot] OHLCV subscribe failed"));
    expect(warnings.map((entry) => entry.meta?.["error"])).toEqual(expect.arrayContaining([
      "4h subscription failed",
      "15m subscription failed",
    ]));
  });
  ...
  it("forces cleanup after the configured graceful drain deadline", async () => {
    ...
    await running;
    expect(logger.entries.some((entry) => entry.message === "[bot] graceful shutdown timeout — force-stopping")).toBe(true);
  });
  ...
  it("keeps the last valid live equity when spot inventory is absent", async () => {
    ...
    const marketMeta = new Map<ExchangeSymbol, MarketMeta>([[symbol, {
      symbol, base: "BTC", quote: "USDC", amountPrecision: 4, pricePrecision: 2,
      minAmount: 0.0001, minCost: 1, isSpot: true,
    }]]);
  ...
  it("treats absent derivative UPL as zero during live reconciliation", async () => {
    ...
    const position: ExchangePosition = {
      symbol, side: "long", quantity: 1, entryPrice: 100, markPrice: 100,
      unrealizedPnl: undefined, updateTimestamp: Date.now(),
    };
    const feed = new ReconciliationFeed(
      [{ currency: "USDC", free: 1_000, total: 1_000 }],
      [{ currency: "USDC", free: 1_000, total: 1_000 }],
      { positions: [position], marketMeta: new Map([[symbol, { symbol, base: "BTC", quote: "USDC", amountPrecision: 4, pricePrecision: 2, minAmount: 0.0001, minCost: 1, isSpot: false }]]) },
    );
    const config = { ...configFor(stateFile), bot: { ...configFor(stateFile).bot, mode: "live" as const } };
    const bot = new Bot({ config, feed, heartbeatIntervalMs: 5, killSwitchEvalIntervalMs: 10_000 });
    const running = bot.start();
  ...
  for (const failure of [new Error("balance Error"), "balance string"] as const) {
    it(`retains live equity and logs ${failure instanceof Error ? "Error" : "string"} reconciliation failures`, async () => {
      ...
      const running = bot.start();
      await waitFor(() => logger.entries.some((entry) => entry.message === "[bot] authoritative equity reconciliation failed"));
      expect(logger.entries.find((entry) => entry.message === "[bot] authoritative equity reconciliation failed")?.meta?.["error"])
  ...
  it("restores every valid persisted position even when the configured capacity is one", async () => {
    const makePosition = (strategy: string, symbol: string): BotState["positions"][number] => ({
      id: `${strategy}:${symbol}:long`, strategy, symbol, side: "long", quantity: 0.01,
      entryPrice: 100, currentPrice: 100, leverage: 10, unrealizedPnl: 0,
      realizedPnl: 0, openedAt: Date.now() - 1_000, notionalUsd: 1,
    });
    const saved: BotState = {
      version: 1, savedAt: Date.now(), equityUsd: 10_000, initialEquityUsd: 10_000,
      realizedPnlUsd: 0,
      positions: [makePosition("first", "BTC/USDC"), makePosition("second", "ETH/USDC")],
      closedTrades: [], inFlightOrderIds: [],
    ...
    expect(logger.entries.some((entry) => entry.message === "[bot] failed to restore position from state — skipping")).toBe(false);
    await bot.stop();
    await running;
  });
  ...
  it("publishes telemetry for restored positions and clamps a nonpositive initial-equity projection", async () => {
    const saved: BotState = {
      version: 1, savedAt: Date.now(), equityUsd: 0, initialEquityUsd: 10_000,
      realizedPnlUsd: 0,
      positions: [{
        id: "telemetry:BTC/USDC:long", strategy: "telemetry", symbol: "BTC/USDC", side: "long",
        quantity: 1, entryPrice: 10_001, currentPrice: 1, leverage: 10,
        unrealizedPnl: -10_000, realizedPnl: 0, openedAt: Date.now() - 1_000, notionalUsd: 10_001,
      }],
      closedTrades: [], inFlightOrderIds: [],
      counters: { placed: 0, filled: 0, cancelled: 0, rejected: 0 },
    };
    ...
    const feed = new MockExchangeFeed();
    const bot = new Bot({
      config: configFor(stateFile), feed,
    ...
    await waitFor(() => {
      ...
      try { return readFileSync(logFile, "utf8").includes("unrealizedPnlUsd"); }
      catch { return false; }
    });
    // eslint-disable-next-line security/detect-non-literal-fs-filename -- telemetry file under per-test mkdtemp path
