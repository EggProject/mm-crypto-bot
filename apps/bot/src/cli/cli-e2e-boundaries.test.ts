...
describe("CLI end-to-end", () => {
  it("runHeadless receives a real subprocess SIGTERM and exits after cleanup", async () => {
    const temporaryDirectory = mkdtempSync(path.join(tmpdir(), "mm-bot-signal-e2e-"));
    const stateFile = path.join(temporaryDirectory, "state.json");
    const readyFile = path.join(temporaryDirectory, "ready");
    const cleanupFile = path.join(temporaryDirectory, "cleanup");
  ...
  it("runHeadless returns one when subprocess SIGTERM cleanup fails", async () => {
    const temporaryDirectory = mkdtempSync(path.join(tmpdir(), "mm-bot-failed-signal-e2e-"));
    const stateFile = path.join(temporaryDirectory, "state.json");
    const readyFile = path.join(temporaryDirectory, "ready");
