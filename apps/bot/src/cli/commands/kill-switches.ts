 ...
 */
export interface KillSwitchesCommandDependencies {
  readonly loadConfig: (path: string | undefined) => BotConfig;
  readonly resolveRuntimeRoot: () => RuntimeRootResolution;
}

const DEFAULT_KILL_SWITCHES_COMMAND_DEPENDENCIES: KillSwitchesCommandDependencies = {
  loadConfig: (configPath) => loadBotConfig(configPath),
  resolveRuntimeRoot: resolveDefaultRuntimeRoot,
};

export function createKillSwitchesCommand(
  overrides: Partial<KillSwitchesCommandDependencies> = {},
): SubcommandHandler {
