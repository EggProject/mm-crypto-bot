...
// derived from the existing WebSocket `state` / `snapshot` messages
// via the `useBotStatus` hook (see `lib/use-bot-status.ts`). The
// previous design polled `GET /api/status` every 1 second — the user
// mandate: "a backendnek kene ertesiteni es nem a frontendnek
// keregetni!". The WS push is now the primary source of truth; the
// HTTP `/api/status` endpoint is only used as a one-shot bootstrap
// fetch on mount + a 30s slow-poll fallback when the WS is
// disconnected (the slow poll lives inside the hook, not here).
...
export function App(): React.JSX.Element {
  ...
  // Phase 81: the bot's high-level status. The `useBotStatus` hook
  // subscribes to the WS `state` / `snapshot` messages, fires a
  // one-shot HTTP bootstrap on mount, and falls back to a 30s slow
  // poll when the WS is disconnected. The 1s polling from Phase 69
  // is GONE — the backend now pushes the data via the WS.
  ...
  // Phase 81: the bot's high-level status comes from `useBotStatus()`
  // (the WS-driven hook). The previous Phase 69 design kept a
  // local `botStatus` state + a 1s polling `useEffect`; both are
  // removed because the backend now pushes the data via the WS.
  //
  // The clock below re-renders the banner every second so the
  // uptime / last-update labels stay fresh without polling the
  // bot. The `now` value is used in `buildStatusBannerText`'s
  // `formatUptime` + `formatLastUpdate` calls.
  ...
  // -----------------------------------------------------------------
  // Phase 81: a 1-second clock that re-renders the banner so the
  // uptime + last-update labels stay fresh. The bot status changes
  // are pushed by the backend via the WS (no polling); the
  // wall-clock ticks every second to keep the "uptime Xs" text
  // up-to-date.
  // -----------------------------------------------------------------
  useEffect(() => {
