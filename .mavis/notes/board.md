---
description: Project board — mm-crypto-bot. Updated 2026-07-20 22:00 Budapest — Phase 60.1+60.2 CLOSED (PR #178 commits 41f1a99+1b0a265+972d0a1). E2E coverage 77.52% / 65.15% / 74.8% (L/B/F) local after Phase 60.2 Object.assign fix; pre-fix 80.88% / 62.03% / 73.07% (Phase 60.1) — **LINES 80% MANDATE MET LOCALLY** (run-to-run variance). Per-file wins: ControlBar.tsx 71.42%/0%/50% → **100%/100%/100%** (Object.assign shallow-merge fix), chart-card-helpers.ts 76.78%/66%/79.41% → 86.2%/85.71%/88.23%, ChartCard.tsx 81.03%/59.09%/70% → 94.44%/64.44%/94.44% (JSX ternary extraction + retainLines: true). 3 sub-agents (React refactor, SWC/V8 investigation, ws-client dispatch tests) all completed; SWC switch obsoleted by retainLines: true (Babel fix per vitejs/vite-plugin-react PR #246). 80/80/80 design target: LINES MET locally, BRANCHES -14.85pp gap (structural: ws-client state machine + lightweight-charts v5 API + React 19 useEffect ordering in 3-WS architecture).
---
