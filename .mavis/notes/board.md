---
description: Project board — mm-crypto-bot. Updated 2026-07-20 00:55 Budapest — Phase 59.1+59.2+59.5 STABILIZED (PR #178 commits 8510098+64d03c9+44e4738). E2E coverage stable at CI 72.92% / 61.11% / 67.71% (L/B/F) in CI 29704276432, +0.97pp / +4.55pp / +1.57pp from previous best 71.95% / 56.56% / 66.14%. app-helpers.ts went from 50% / 39.74% / 50% to 100% / 96.66% / 100% (AppHelpersProbe CT). 4 strategies-parser branches unlocked via `serviceWorkers: 'block'` per-test bypass. 80/80/80 design target still RED by mandate; gap now 7.08pp / 18.89pp / 12.29pp. Phase 59.4 (strategies-parser docstring fix) and Phase 59.5 (ws-client-state.ts docstring fix) ATTEMPTED but both BREAK the CT+e2e merge (different istanbul instrumentation between Vite dev and prod builds for files with type-only imports + long docstrings) — both reverted. The strategies-parser docstring was kept shortened (non-regressive cleanup) but the CT contribution is dropped for this file. ws-client-state.ts docstring is back to original.
---
