---
description: Project board — mm-crypto-bot. Updated 2026-07-21 01:15 Budapest — **Phase 61+62 CLOSED — 80/80/80 MANDATE MET ON ALL 3 METRICS** (PR #178 commits 41f1a99+1b0a265+972d0a1+54b1dc5+a2674e2+ee9fc24+2e76a3d). E2E coverage 90.85% / 84.34% / 86.61% (L/B/F), +18.90pp / +27.78pp / +20.47pp from Phase 58 baseline (71.95%/56.56%/66.14%). Per-file wins: realtime-batcher.ts 60%→100% B (+40pp), App.tsx 41.66%→75% B (+33.34pp), strategies-parser.ts 0%→100% B (+100pp), theme.ts 57.14%→71.42% B (+14.28pp), subscription.ts 66.66%→77.77% B (+11.11pp), ControlBar.tsx 0%→100% B (Phase 60.2 Object.assign fix), ws-client-state.ts 68.96%→79.31% B (+10.35pp). CI run confirms `✓ Coverage OK: 90.85% lines / 84.34% branches / 86.61% functions (thresholds 80/80/80)`. Critical fix: e2e `_helpers/coverage.ts` had 3 separate `Object.assign` shallow-merge bugs (parallel to CT-side fix in Phase 60.2) — fixed in 3 places with `createCoverageMap().merge()`. Strategies-parser 4 defensive branches unlocked via the deep-merge (not serviceWorkers: 'block' as previously assumed). 8 new e2e tests in 62-targeted-branch-coverage.spec.ts (realtime-batcher pushMany/flushNow, ws-client-state closedByCaller/no-data, etc). PR #178 is now MERGE-ABLE.
---
