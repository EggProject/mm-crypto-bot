...
function parseGrids(value: unknown): ParsedGrids {
  if (!isObject(value) || !isObject(value["production"]) || !isObject(value["ablations"]) || !isObject(value["optionalAdapters"])) {
  ...
  if (!isObject(rawDpc) || !isObject(rawSolFlip) || !isObject(rawDrc) || !isObject(rawPivot) || !isObject(rawOhlc) || !isObject(rawOverlay)) {
  ...
  const emaPairs = requireArray(rawOhlc["emaPairs"], "ohlc_trend.emaPairs").map((pair): readonly [number, number] => {
    const values = numberArray(pair, "ohlc_trend.emaPairs[]");
    if (values.length !== 2 || values[0] === undefined || values[1] === undefined) throw new Error("Hibás EMA pár");
  ...
  return {
    ...
    solFlip: {
      ...
      extremeSigmaThreshold: numberArray(rawSolFlip["extremeSigmaThreshold"], "solFlip.extremeSigmaThreshold"),
...
function gitRevision(): string | null {
  const result = Bun.spawnSync(["git", "rev-parse", "HEAD"], { cwd: REPO_ROOT, stdout: "pipe", stderr: "ignore" });
...
function selectSplits(splits: readonly Split[], requestedPhases: readonly string[]): readonly Split[] {
  ...
  if (selected.length !== requestedPhases.length) throw new Error(`Ismeretlen vagy ismételt phase: ${requestedPhases.join(",")}`);
  return selected;
}
...
export async function buildSearchManifest(
  ...
  for (const combination of matrix) {
    ...
    const overlayMask = combination.mask === "10001"
      ? "dpc-regime"
      : combination.mask === "10010"
        ...
        : combination.mask === "10011"
          ...
          : null;
    if (overlayMask !== null) {
      for (const split of selectedSplits) for (const symbol of combination.validSymbols) for (const minConsensus of grids.overlay.minConsensus) for (const cap of grids.overlay.maxPositionPctEquity) {
        ...
        if (!overlayAdapter.ready) {
          jobs.push({ runId, componentMask: combination.mask, enabledComponents: combination.enabled, strategyId: "overlay_combination", status: "UNSUPPORTED_ADAPTER", reason: overlayAdapter.reason, parameters, split, command: null, rawOutput: null, inputFiles: [], metrics: nullMetrics() });
          continue;
        }
        const rawOutput = outputFor(outputDir, runId);
        const usesSolFlip = overlayMask.includes("solflip");
        const inputFiles = [`data/ohlcv/binance_${symbolId(symbol)}_15m.csv`, `data/ohlcv/binance_${symbolId(symbol)}_4h.csv`, `data/ohlcv/binance_${symbolId(symbol)}_1d.csv`, ...(usesSolFlip ? [grids.overlay.fundingInput] : [])];
        jobs.push(readyJob({
          runId,
          componentMask: combination.mask,
          enabledComponents: combination.enabled,
          ...
          rawOutput,
          inputFiles,
          command: ["bun", "run", OVERLAY_ADAPTER.runner, `--mask=${overlayMask}`, `--symbol=${symbol}`, `--start=${split.start}`, `--end=${split.end}`, `--min-consensus=${minConsensus}`, `--max-position-pct-equity=${cap}`, `--funding-input=${resolve(REPO_ROOT, grids.overlay.fundingInput)}`, `--output=${rawOutput}`],
        }));
      }
      continue;
    }
    for (const split of selectedSplits) for (const symbol of dpc.symbols) for (const minConsensus of dpc.minConsensus) for (const cap of dpc.maxPositionPctEquity) {
      ...
      jobs.push(readyJob({
        ...
        parameters: { symbol, timeframe: dpc.timeframe, initialEquityUsd: dpc.initialEquityUsd, minConsensus, maxPositionPctEquity: cap },
        split,
        rawOutput,
        inputFiles: [`data/ohlcv/binance_${symbolId(symbol)}_15m.csv`, `data/ohlcv/binance_${symbolId(symbol)}_4h.csv`, `data/ohlcv/binance_${symbolId(symbol)}_1d.csv`],
        command: ["bun", "run", "packages/backtest-tools/src/cli/run-donchian-pivot-composition.ts", `--symbol=${symbol}`, `--timeframe=${dpc.timeframe}`, `--equity=${dpc.initialEquityUsd}`, `--min-consensus=${minConsensus}`, `--max-position-pct-equity=${cap}`, `--start=${split.start}`, `--end=${split.end}`, `--output=${rawOutput}`],
      }));
    }
  }
  return manifestEnvelope(requestedPhases, matrix.map((row) => row.mask), jobs, outputDir, true);
}
...
async function probeOverlayAdapter(): Promise<{ readonly ready: boolean; readonly reason: string }> {
  ...
  const help = Bun.spawnSync(["bun", "run", OVERLAY_ADAPTER.runner, "--help"], { cwd: REPO_ROOT, stdout: "pipe", stderr: "pipe" });
  ...
  if (help.exitCode !== 0 || !OVERLAY_ADAPTER.requiredHelpTokens.every((token) => helpText.includes(token))) {
    return { ready: false, reason: "Adapter inaktív: a --help szerződés vagy a kötelező --smoke/--mask/--symbol/--output/--start/--end flag hiányzik" };
  }
  const temp = await mkdtemp(resolve(tmpdir(), "overlay-adapter-smoke-"));
  const smokeOutput = resolve(temp, "smoke.json");
  try {
    const smoke = Bun.spawnSync(["bun", "run", OVERLAY_ADAPTER.runner, "--smoke", `--output=${smokeOutput}`], { cwd: REPO_ROOT, stdout: "pipe", stderr: "pipe" });
    if (smoke.exitCode !== 0) return { ready: false, reason: "Adapter inaktív: a CLI smoke futás hibával állt le" };
    const document = JSON.parse(await readFile(smokeOutput, "utf8")) as unknown;
    if (!isObject(document)) return { ready: false, reason: "Adapter inaktív: a smoke output nem JSON objektum" };
    return { ready: true, reason: "A később telepített overlay adapter help és smoke szerződése érvényes" };
  } catch (error) {
    return { ready: false, reason: `Adapter inaktív: hibás smoke output (${error instanceof Error ? error.message : String(error)})` };
  } finally {
...
export async function buildRunnableManifest(
  ...
  for (const strategyId of requestedStrategies) {
    if (strategyId === "overlay_combination") {
      ...
      for (const split of selectedSplits) for (const mask of g.masks) for (const symbol of g.symbols) for (const minConsensus of g.minConsensus) for (const cap of g.maxPositionPctEquity) {
        ...
        if (usesSolFlip && symbol !== "SOL/USDT") {
          jobs.push({ runId, componentMask: mask, enabledComponents: mask.split("-"), strategyId, status: "INVALID_MASK", reason: `${mask} csak SOL/USDT szimbólummal érvényes; BTC/ETH nem kezelhető no-opként`, parameters, split, command: null, rawOutput: null, inputFiles: [], metrics: nullMetrics() });
          continue;
        }
        if (!g.supportedPhases.includes(split.id)) {
          jobs.push({ runId, componentMask: mask, enabledComponents: mask.split("-"), strategyId, status: "UNSUPPORTED_WINDOW", reason: `Az overlay CLI jelenleg csak ezt támogatja: ${g.supportedPhases.join(",")}`, parameters, split, command: null, rawOutput: null, inputFiles: [], metrics: nullMetrics() });
          continue;
        }
        if (!adapter.ready) {
          jobs.push({ runId, componentMask: mask, enabledComponents: mask.split("-"), strategyId, status: "UNSUPPORTED_ADAPTER", reason: adapter.reason, parameters, split, command: null, rawOutput: null, inputFiles: [], metrics: nullMetrics() });
          continue;
        }
        const rawOutput = outputFor(outputDir, runId);
        const inputFiles = [`data/ohlcv/binance_${symbolId(symbol)}_15m.csv`, `data/ohlcv/binance_${symbolId(symbol)}_4h.csv`, `data/ohlcv/binance_${symbolId(symbol)}_1d.csv`, ...(usesSolFlip ? [g.fundingInput] : [])];
        jobs.push(readyJob({ runId, componentMask: mask, enabledComponents: mask.split("-"), strategyId, parameters, split, rawOutput, inputFiles, command: ["bun", "run", OVERLAY_ADAPTER.runner, `--mask=${mask}`, `--symbol=${symbol}`, `--start=${split.start}`, `--end=${split.end}`, `--min-consensus=${minConsensus}`, `--max-position-pct-equity=${cap}`, `--funding-input=${resolve(REPO_ROOT, g.fundingInput)}`, `--output=${rawOutput}`] }));
      }
      continue;
    }
    if (!RUNNABLE_STRATEGIES.includes(strategyId as typeof RUNNABLE_STRATEGIES[number])) throw new Error(`Ismeretlen strategy: ${strategyId}`);
    for (const split of selectedSplits) {
      if (strategyId === "donchian_pivot_composition") {
        const g = grids.dpc;
        for (const symbol of g.symbols) for (const minConsensus of g.minConsensus) for (const cap of g.maxPositionPctEquity) {
          const runId = `dpc-${split.id}-${symbolId(symbol)}-${minConsensus}of2-cap${compact(cap)}`;
          const rawOutput = outputFor(outputDir, runId);
          jobs.push(readyJob({ runId, componentMask: "10000", enabledComponents: [strategyId], strategyId, parameters: { symbol, timeframe: g.timeframe, initialEquityUsd: g.initialEquityUsd, minConsensus, maxPositionPctEquity: cap }, split, rawOutput, inputFiles: [`data/ohlcv/binance_${symbolId(symbol)}_15m.csv`, `data/ohlcv/binance_${symbolId(symbol)}_4h.csv`, `data/ohlcv/binance_${symbolId(symbol)}_1d.csv`], command: ["bun", "run", "packages/backtest-tools/src/cli/run-donchian-pivot-composition.ts", `--symbol=${symbol}`, `--timeframe=${g.timeframe}`, `--equity=${g.initialEquityUsd}`, `--min-consensus=${minConsensus}`, `--max-position-pct-equity=${cap}`, `--start=${split.start}`, `--end=${split.end}`, `--output=${rawOutput}`] }));
        }
      } else if (strategyId === "donchian_range_channel") {
        const g = grids.drc;
        for (const symbol of g.symbols) for (const donchianPeriod of g.donchianPeriod) for (const adxTrendThreshold of g.adxTrendThreshold) {
          const runId = `drc-${split.id}-${symbolId(symbol)}-dc${donchianPeriod}-adx${compact(adxTrendThreshold)}`;
          const rawOutput = outputFor(outputDir, runId);
          jobs.push(readyJob({ runId, componentMask: "ablation", enabledComponents: [strategyId], strategyId, parameters: { symbol, timeframe: g.timeframe, initialEquityUsd: g.initialEquityUsd, donchianPeriod, adxTrendThreshold }, split, rawOutput, inputFiles: [`data/ohlcv/binance_${symbolId(symbol)}_15m.csv`, `data/ohlcv/binance_${symbolId(symbol)}_4h.csv`, `data/ohlcv/binance_${symbolId(symbol)}_1d.csv`], command: ["bun", "run", "packages/backtest-tools/src/cli/run-donchian-range-baseline.ts", `--symbol=${symbol}`, `--timeframe=${g.timeframe}`, `--equity=${g.initialEquityUsd}`, `--donchian-period=${donchianPeriod}`, `--adx-trend-threshold=${adxTrendThreshold}`, `--start=${split.start}`, `--end=${split.end}`, `--output=${rawOutput}`] }));
        }
      } else if (strategyId === "pivot_point_grid") {
        const g = grids.pivot;
        for (const symbol of g.symbols) for (const cap of g.maxPositionPctEquity) {
          const runId = `pivot-${split.id}-${symbolId(symbol)}-cap${compact(cap)}`;
          const rawOutput = outputFor(outputDir, runId);
          jobs.push(readyJob({ runId, componentMask: "ablation", enabledComponents: [strategyId], strategyId, parameters: { symbol, timeframe: g.timeframe, initialEquityUsd: g.initialEquityUsd, maxPositionPctEquity: cap }, split, rawOutput, inputFiles: [`data/ohlcv/binance_${symbolId(symbol)}_15m.csv`, `data/ohlcv/binance_${symbolId(symbol)}_4h.csv`, `data/ohlcv/binance_${symbolId(symbol)}_1d.csv`], command: ["bun", "run", "packages/backtest-tools/src/cli/run-pivot-grid-baseline.ts", `--symbol=${symbol}`, `--timeframe=${g.timeframe}`, `--equity=${g.initialEquityUsd}`, `--max-position-pct-equity=${cap}`, `--start=${split.start}`, `--end=${split.end}`, `--output=${rawOutput}`] }));
        }
      } else if (strategyId === "ohlc_trend") {
        const g = grids.ohlcTrend;
        for (const symbol of g.symbols) for (const timeframe of g.timeframe) for (const [fastEma, slowEma] of g.emaPairs) for (const atrStopMultiplier of g.atrStopMultiplier) for (const rewardToRisk of g.rewardToRisk) for (const crossLookback of g.crossLookback) {
          ...
          const rawOutput = outputFor(outputDir, runId);
          jobs.push(readyJob({ runId, componentMask: "non-production", enabledComponents: [strategyId], strategyId, parameters: { symbol, timeframe, initialEquityUsd: g.initialEquityUsd, fastEma, slowEma, atrStopMultiplier, rewardToRisk, crossLookback }, split, rawOutput, inputFiles: [`data/ohlcv/binance_${symbolId(symbol)}_${timeframe}.csv`], command: ["bun", "run", "packages/backtest-tools/src/cli/run-ohlc-trend-baseline.ts", `--symbol=${symbol}`, `--timeframe=${timeframe}`, `--equity=${g.initialEquityUsd}`, `--fast-ema=${fastEma}`, `--slow-ema=${slowEma}`, `--atr-stop-multiplier=${atrStopMultiplier}`, `--reward-to-risk=${rewardToRisk}`, `--cross-lookback=${crossLookback}`, `--start=${split.start}`, `--end=${split.end}`, `--output=${rawOutput}`] }));
        }
      } else {
        const g = grids.solFlip;
        for (const signFlipWindowDays of g.signFlipWindowDays) for (const extremeSigmaThreshold of g.extremeSigmaThreshold) for (const persistenceDays of g.persistenceDays) for (const volWindowDays of g.volWindowDays) {
          ...
          const rawOutput = outputFor(outputDir, runId);
          jobs.push(readyJob({ runId, componentMask: "00010", enabledComponents: [strategyId], strategyId, parameters: { signFlipWindowDays, extremeSigmaThreshold, persistenceDays, volWindowDays, pnlApplicable: false }, split, rawOutput, inputFiles: [g.input], command: ["bun", "run", "packages/backtest-tools/src/cli/run-sol-flip-funding-replay.ts", `--input=${resolve(REPO_ROOT, g.input)}`, `--sign-flip-window-days=${signFlipWindowDays}`, `--extreme-sigma-threshold=${extremeSigmaThreshold}`, `--persistence-days=${persistenceDays}`, `--vol-window-days=${volWindowDays}`, `--start=${split.start}`, `--end=${split.end}`, `--output=${rawOutput}`] }));
        }
      }
    }
  }
...
function manifestEnvelope(phases: readonly string[], representedMasks: readonly string[], jobs: readonly SearchJob[], outputDir: string, dryRun: boolean, strategies?: readonly string[]): Readonly<Record<string, unknown>> {
  return {
    schemaVersion: 2,
    dryRun,
    ...
    phases,
    ...(strategies === undefined ? { combinationCount: representedMasks.length, representedMasks } : { strategies }),
...
export function parseRunSearchArgs(argv: readonly string[]): CliOptions {
  ...
  for (const arg of argv) {
    ...
    if (!["execute", "strategy", "phase", "concurrency", "resume", "output-dir", "output"].includes(name)) throw new Error(`Ismeretlen argumentum: --${name}`);
  ...
  if (!Number.isInteger(concurrency) || concurrency < 1 || concurrency > 64) throw new Error("--concurrency 1 és 64 közötti egész legyen");
  const rawStrategies = (values.get("strategy") ?? "production").split(",").map((item) => item.trim()).filter(Boolean);
  const strategies = rawStrategies.flatMap((item) => item === "all" || item === "all-runnable" ? [...RUNNABLE_STRATEGIES, "overlay_combination"] : [STRATEGY_ALIASES[item] ?? item]);
  if (new Set(strategies).size !== strategies.length) throw new Error("A --strategy lista ismételt elemet tartalmaz");
  const phases = (values.get("phase") ?? "is,validation,oos").split(",").map((item) => item.trim()).filter(Boolean);
  ...
  return { execute, strategies, phases, concurrency, resume: parseBoolean("resume", values.get("resume") ?? "true"), outputDir, output };
}
...
export async function main(argv: readonly string[] = process.argv.slice(2)): Promise<void> {
  ...
  manifest = { ...manifest, dryRun: false, executedAt: new Date().toISOString(), codeRevision: code.codeRevision, dirtyDiffSha256: code.dirtyDiffSha256, jobs: executedJobs };
  await writeText(options.output, `${JSON.stringify(manifest, null, 2)}\n`);
  console.log(`EXECUTE: ${executedJobs.length} sor, ${failed.length} FAILED, concurrency=${options.concurrency} → ${options.output}`);
