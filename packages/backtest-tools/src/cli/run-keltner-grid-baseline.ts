#!/usr/bin/env bun
// packages/backtest-tools/src/cli/run-keltner-grid-baseline.ts — Keltner Volatility-Adaptive Grid baseline
//
// Phase 15 Track D — Keltner Grid strategy baseline backtest (M5).
// Grid strategy anchored to Keltner channel (EMA20 ± 1.5×ATR), auto-resizes
// with realized volatility. Strategy class lives in `@mm-crypto-bot/core`
// and is implemented by Track C.
//
// Használat:
//   bun run packages/backtest-tools/src/cli/run-keltner-grid-baseline.ts --symbol=BTC/USDT --timeframe=5m --output=backtest-results/phase15-keltner-grid-btc-5m.json
...
import { KeltnerGridStrategy, DEFAULT_KELTNER_GRID_CONFIG } from "@mm-crypto-bot/core";
...
function parseArgs(): CliArgs {
  ...
  let outputPath = "backtest-results/phase15-keltner-grid-btc-5m.json";
  for (const arg of args) {
    if (arg.startsWith("--symbol=")) {
      symbol = arg.slice("--symbol=".length);
    } else if (arg.startsWith("--timeframe=")) {
      const tf = arg.slice("--timeframe=".length) as Timeframe;
      // Phase 15 — Keltner Grid baseline runs on M5.
      if (tf !== "5m") {
        throw new Error(`Keltner Grid baseline requires 5m timeframe, got: ${tf}`);
      }
      timeframe = tf;
...
// Keltner Grid timeline — HTF=1d, MTF=1h, LTF=5m.
function timeframesForKeltnerGrid(ltf: Timeframe): { htf: Timeframe; mtf: Timeframe; ltf: Timeframe } {
  if (ltf === "5m") return { htf: "1d", mtf: "1h", ltf: "5m" };
  throw new Error(`Keltner Grid baseline supports 5m only, got: ${ltf as string}`);
}
...
async function main(): Promise<void> {
  ...
  const tf = timeframesForKeltnerGrid(args.timeframe);
  const dataDir = resolve(import.meta.dir, "..", "..", "..", "..", "data", "ohlcv");
  const feed = new CsvExchangeFeed(dataDir) as unknown as ExchangeFeed;
  const strategy = new KeltnerGridStrategy(DEFAULT_KELTNER_GRID_CONFIG);

  // 2024-01-01 → today.
  const startTime = new Date(Date.UTC(2024, 0, 1));
  const endTime = new Date();

  console.log(`[keltner-grid] symbol=${args.symbol} ltf=${args.timeframe}`);
  console.log(`[keltner-grid] timeframes: htf=${tf.htf} mtf=${tf.mtf} ltf=${tf.ltf}`);
  console.log(`[keltner-grid] period: ${startTime.toISOString()} → ${endTime.toISOString()}`);
  console.log(`[keltner-grid] initial equity: $${args.initialEquity}`);
  ...
  console.log(`\n=== RESULTS keltner-grid ${args.symbol} ${args.timeframe} ===`);
  ...
  console.log(`\n[keltner-grid] Saved: ${args.outputPath}`);
}

main().catch((err: unknown) => {
  console.error("[keltner-grid] FATAL:", err);
  process.exit(1);
});
