#!/usr/bin/env bun
// packages/backtest-tools/src/cli/run-donchian-baseline.ts — Donchian volatility breakout baseline
//
// Phase 5 — Donchian breakout stratégia baseline backtest (StrategyArena / Arconomy / Arxum referencia).
// Az engine-bug fix a Phase 4 PR #10 merge után a main-en van.
...
//   bun run packages/backtest-tools/src/cli/run-donchian-baseline.ts --symbol=ETH/USDT --timeframe=4h
//   bun run packages/backtest-tools/src/cli/run-donchian-baseline.ts --vol=2.0 (volume multiplier)
//   bun run packages/backtest-tools/src/cli/run-donchian-baseline.ts --no-htf-filter
...
function parseArgs(): CliArgs {
  ...
  let outputPath = "backtest-results/baseline-donchian-btc-1h.json";
  ...
  return { symbol, timeframe, initialEquity, volumeConfirmMultiplier, useHtfTrendFilter, outputPath };
}
...
async function main(): Promise<void> {
  ...
  console.log(`[donchian-baseline] symbol=${args.symbol} ltf=${args.timeframe}`);
  console.log(`[donchian-baseline] vol=${args.volumeConfirmMultiplier}×avg, htf-filter=${args.useHtfTrendFilter}`);
  console.log(`[donchian-baseline] period: ${startTime.toISOString()} → ${endTime.toISOString()}`);
  ...
  console.log(`\n=== DONCHIAN BREAKOUT RESULTS ${args.symbol} ${args.timeframe} ===`);
  ...
  // Trade-count reality check (strategy-selection §4.5.2: expected 30-100 / 30 hó / symbol)
  if (result.totalTrades === 0) {
    console.warn(`[donchian-baseline] ⚠ DEVIATION FROM ESTIMATED RANGE: 0 trades (expected 30-100 / 30 hó / symbol — strategy-selection §4.5.2)`);
  } else if (result.totalTrades > 5000) {
    console.warn(`[donchian-baseline] ⚠ DEVIATION FROM ESTIMATED RANGE: ${result.totalTrades} trades (expected 30-100 / 30 hó / symbol — strategy-selection §4.5.2)`);
  } else {
    console.log(`[donchian-baseline] ✓ Trade-count (${result.totalTrades}) within estimated range (30-100)`);
  }
  ...
  console.log(`[donchian-baseline] Saved: ${absOutput}`);
}

main().catch((err: unknown) => {
  console.error("[donchian-baseline] FATAL:", err);
  process.exit(1);
});
