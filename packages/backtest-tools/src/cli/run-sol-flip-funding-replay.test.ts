...
import {
  ...
  parseFundingCsv,
  replayFunding,
} from "./run-sol-flip-funding-replay.js";
...
describe("run-sol-flip-funding-replay — real Binance funding CSV", () => {
  it("valós funding sorokon regime/risk metrikát ad, PnL-t és DD-t nem fabrikál", () => {
    ...
    const rows = parseFundingCsv(readFileSync(REAL_FUNDING, "utf8"))
      .filter((row) => row.fundingTime >= args.startTime.getTime() && row.fundingTime < args.endTime.getTime());
  ...
  it("a direct CLI auditálható valós-adat outputot ír", async () => {
    ...
    try {
      ...
      expect(parsed.data).toMatchObject({ sourceKind: "downloaded_binance_funding_csv", synthetic: false, sampleCount: 456, coverageRatio: 1 });
