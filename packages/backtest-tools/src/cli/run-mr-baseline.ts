#!/usr/bin/env bun
// packages/backtest-tools/src/cli/run-mr-baseline.ts — Mean-Reversion baseline
//
// Phase 4 — az új MeanReversionBbStrategy futtatása a Phase 1 OHLCV adatokon,
// a Phase 1-3 MTF-TKC baseline-nál sokkal több trade-et generálva.
//
// Használat:
//   bun run packages/backtest-tools/src/cli/run-mr-baseline.ts
//   bun run packages/backtest-tools/src/cli/run-mr-baseline.ts --symbol=ETH/USDT --timeframe=4h
//   bun run packages/backtest-tools/src/cli/run-mr-baseline.ts --stop=0.005
...
import { MeanReversionBbStrategy } from "@mm-crypto-bot/core";
...
function parseArgs(): CliArgs {
  ...
  let stopLossPct = 0.01;
  let outputPath = "backtest-results/mr-baseline-btc-1h.json";
  for (const arg of args) {
    if (arg.startsWith("--symbol=")) {
      ...
      stopLossPct = Number(arg.slice("--stop=".length));
    } else if (arg.startsWith("--output=")) {
      outputPath = arg.slice("--output=".length);
    }
  }
  return { symbol, timeframe, initialEquity, stopLossPct, outputPath };
}
...
// bybit.eu SPOT 1:10 — ugyanaz a cost-model mint a Phase 1-3 baseline-ban
...
async function main(): Promise<void> {
  ...
  const strategy = new MeanReversionBbStrategy({ stopLossPct: args.stopLossPct });

  console.log(`[mr-baseline] symbol=${args.symbol} ltf=${args.timeframe}`);
  console.log(`[mr-baseline] stop=${(args.stopLossPct * 100).toFixed(2)}%`);
  console.log(`[mr-baseline] period: ${startTime.toISOString()} → ${endTime.toISOString()}`);
  ...
  const result: BacktestResult = await runBacktest({
    ...
    positionSize: {
      riskPerTrade: 0.01,
      kellyFraction: 0.25,
      maxDrawdown: 0.5, // disabled for diagnostics
  ...
  console.log(`\n=== MR-BB RESULTS ${args.symbol} ${args.timeframe} ===`);
  ...
  console.log(
    `\n[mr-baseline] ${result.totalTrades >= 30 && monthlyReturn > 0 ? "✓" : "⚠"}` +
      ` sample size ${result.totalTrades} trades, monthly ${(monthlyReturn * 100).toFixed(2)}%/mo`,
  );
  ...
  console.log(`[mr-baseline] Saved: ${absOutput}`);
}

main().catch((err: unknown) => {
  console.error("[mr-baseline] FATAL:", err);
  process.exit(1);
});
