...
describe("run-dpc-overlay-combination — production overlay replay on real CSV data", () => {
  it("exposes exactly the four requested masks and rejects SOLFlip as a BTC/ETH no-op", () => {
    expect(OVERLAY_MASKS).toEqual([
      "dpc",
      "dpc-solflip",
      "dpc-regime",
      "dpc-solflip-regime",
    ]);
    expect(() => args("dpc-solflip", "BTC/USDT", "2024-01-01", "2024-01-02"))
      .toThrow(/INVALID_MASK.*SOL\/USDT/);
    expect(() => args("dpc-solflip-regime", "ETH/USDT", "2024-01-01", "2024-01-02"))
      .toThrow(/INVALID_MASK.*SOL\/USDT/);
  });
  ...
  it("Regime ON consumes genuine DPC direction/executed sizing and changes real position notionals", async () => {
    ...
    expect(regime.overlayMetrics.regimeDirectionSignalsReceived).toBe(regime.overlayMetrics.dpcDirectionSignalsEmitted);
    expect(regime.overlayMetrics.regimeSizingSignalsReceived).toBe(regime.overlayMetrics.dpcSizingSignalsEmitted);
    expect(regime.sizingSignals.some((signal, index) =>
      signal.notional !== plain.sizingSignals[index]?.notional)).toBe(true);
  ...
  it("SOLFlip+Regime ON changes the real SOL trade stream and remains causal", async () => {
    ...
    expect(overlaid.causality.audit.every((event) =>
      event.lastFundingTimeConsumed !== null && event.lastFundingTimeConsumed <= event.decisionTime)).toBe(true);
  ...
  it("publishes the adapter help contract and writes a quick smoke JSON without a backtest", async () => {
    ...
    const [help, helpExit] = await Promise.all([
      new Response(helpChild.stdout).text(),
      helpChild.exited,
    ]);
