...
import type { ExchangeFeeConfig, FillRecord, TradingSignal } from "@mm-crypto-bot/shared";
...
describe("paper barrel index", () => {
  ...
  it("a PaperTraderOptions típus használható a konstruktorban", () => {
    ...
    const pt = new PaperTrader(feed, options);
  ...
  it("preserves the public null no-fill contract through the barrel", async () => {
    const feed = new MockExchangeFeed();
    const trader = new PaperTrader(feed, { initialBalanceQuote: 1000, fee: FEE });
    const holdSignal: TradingSignal = {
      symbol: "BTC/USDT",
      action: "hold",
      confidence: 0.5,
      reason: "public-contract-test",
      generatedAt: Date.now(),
    };
    expectTypeOf(trader.executeSignal(holdSignal)).toEqualTypeOf<Promise<FillRecord | null>>();
    expect(await trader.executeSignal(holdSignal)).toBeNull();

    const fill = await trader.executeSignal({
      ...holdSignal,
      action: "buy",
      suggestedAmount: 0.1,
      suggestedPrice: 100,
