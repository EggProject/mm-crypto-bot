...
export const DEFAULT_NORMAL_MULTIPLIER = 1 as const;
export const DEFAULT_COMPRESSED_MULTIPLIER = 1 as const; // don't fight compression
export const DEFAULT_NO_DATA_MULTIPLIER = 1 as const; // fail-open
...
const noDvolForTimestamp = (): null => null;
...
export interface DvolRegimeSizingPluginState {
  ...
  */
  ...
  /**
  Total `no-data` emissions (fail-open path).
  */
 ...
 *
 * The plugin never scales up beyond the sizing baseline.
 */
export class DvolRegimeSizingPlugin implements StrategyPlugin {
  private _boundSymbol: string | undefined;
  private _bus: SignalBus | undefined;
  ...
  public readonly metadata: StrategyPluginMetadata = {
    ...
    description:
      "BTC options implied-volatility (DVOL) regime sizing. " +
      ...
      "no-data 1.0 fail-open). Aggregate effective-exposure limit is enforced.",
    dependencies: [],
  };
  ...
  constructor(overrides: Partial<DvolRegimeSizingConfig> = {}) {
    this.config = {
      ...
      getDvolForTimestamp: overrides.getDvolForTimestamp ?? noDvolForTimestamp,
    ...
    if (
      ...
      throw new Error(
        `[DvolRegimeSizingPlugin] aggregate effective-exposure limit metadata does not match the configured default.`,
      );
    }
    ...
    if (!Number.isFinite(this.config.baseNotionalUsd) || this.config.baseNotionalUsd <= 0) {
      throw new Error(
        `[DvolRegimeSizingPlugin] baseNotionalUsd must be positive finite, got ${String(this.config.baseNotionalUsd)}`,
      );
    }
    if (this.config.acuteStressThreshold <= this.config.elevatedThreshold) {
      throw new Error(
        `[DvolRegimeSizingPlugin] acuteStressThreshold=${String(this.config.acuteStressThreshold)} must be > elevatedThreshold=${String(this.config.elevatedThreshold)}`,
      );
    }
    if (this.config.elevatedThreshold <= this.config.normalThreshold) {
      throw new Error(
        `[DvolRegimeSizingPlugin] elevatedThreshold=${String(this.config.elevatedThreshold)} must be > normalThreshold=${String(this.config.normalThreshold)}`,
      );
    }
    if (this.config.acuteStressMultiplier < 0 || this.config.acuteStressMultiplier > 1) {
      throw new Error(
        `[DvolRegimeSizingPlugin] acuteStressMultiplier=${String(this.config.acuteStressMultiplier)} must be in [0, 1.0]`,
      );
    }
    if (this.config.elevatedMultiplier < 0 || this.config.elevatedMultiplier > 1) {
      throw new Error(
        `[DvolRegimeSizingPlugin] elevatedMultiplier=${String(this.config.elevatedMultiplier)} must be in [0, 1.0]`,
      );
    }
    if (this.config.normalMultiplier < 0 || this.config.normalMultiplier > 1) {
      throw new Error(
        `[DvolRegimeSizingPlugin] normalMultiplier=${String(this.config.normalMultiplier)} must be in [0, 1.0]`,
      );
    }
    if (this.config.compressedMultiplier < 0 || this.config.compressedMultiplier > 1) {
      throw new Error(
        `[DvolRegimeSizingPlugin] compressedMultiplier=${String(this.config.compressedMultiplier)} must be in [0, 1.0]`,
      );
    }
    if (this.config.noDataMultiplier < 0 || this.config.noDataMultiplier > 1) {
      throw new Error(
        `[DvolRegimeSizingPlugin] noDataMultiplier=${String(this.config.noDataMultiplier)} must be in [0, 1.0]`,
      );
    }
  ...
  private _processSymbol(symbol: string, timestampMs: number): void {
    ...
    const rawDvol = fromOverride ?? this.config.getDvolForTimestamp(timestampMs);
    const dvol = rawDvol ?? undefined;

    let regime: DvolRegime;
    let volMultiplier: number;
    if (dvol === undefined || !Number.isFinite(dvol)) {
    ...
    this._incrementRegimeCount(regime);
  ...
  private _incrementRegimeCount(regime: DvolRegime): void {
    switch (regime) {
      case "acute-stress": {
        this.state.regimeCounts["acute-stress"] += 1;
        return;
      }
      case "elevated": {
        this.state.regimeCounts.elevated += 1;
        return;
      }
      case "normal": {
        this.state.regimeCounts.normal += 1;
        return;
      }
      case "compressed": {
        this.state.regimeCounts.compressed += 1;
        return;
      }
      case "no-data": {
        this.state.regimeCounts["no-data"] += 1;
      }
    }
  }
  ...
  subscribe(bus: SignalBus): void {
    this._boundSymbol = bus.scopeSymbol;
    this._bus = bus;
    this._wired = true;
  }
  ...
  onBar(bar: Bar, _state: PluginState): void {
    ...
    const symbols = this._boundSymbol === undefined ? this.config.enabledSymbols : [this._boundSymbol];
  ...
  validateConfig(config: unknown): Result<void, ConfigError> {
    ...
    if (!isRecord(config)) return makeError("config", "must be object");
    const c = config;
    if (c["baseNotionalUsd"] !== undefined) {
      const bn = c["baseNotionalUsd"];
      if (typeof bn !== "number" || !Number.isFinite(bn) || bn <= 0) {
        return makeError("baseNotionalUsd", "must be positive finite number", bn);
      }
    }
    if (c["getDvolForTimestamp"] !== undefined && typeof c["getDvolForTimestamp"] !== "function") {
      return makeError("getDvolForTimestamp", "must be a function");
    }
    return ok(undefined);
  }
