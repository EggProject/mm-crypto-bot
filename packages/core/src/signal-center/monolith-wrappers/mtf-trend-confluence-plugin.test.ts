// packages/core/src/signal-center/monolith-wrappers/mtf-trend-confluence-plugin.test.ts
// — Phase 13 Track A
//
// Test suite for MtfTrendConfluencePlugin — ≥5 tests covering:
...
import {
  ...
  createMtfTrendConfluencePlugin,
  DEFAULT_MTF_TREND_CONFLUENCE_PLUGIN_CONFIG,
} from "./mtf-trend-confluence-plugin.js";
import type { Bar } from "../types.js";

const mkBus = (): SignalBus => new SignalBus({ mode: "backtest" });

const wirePlugin = (p: MtfTrendConfluencePlugin): SignalBus => {
  const bus = mkBus();
  p.subscribe(bus);
  return bus;
};
...
describe("MtfTrendConfluencePlugin", () => {
  it("construction with default config succeeds", () => {
    const p = new MtfTrendConfluencePlugin();
  ...
  it("metadata declares maxLeverage=10 (1:10 HARD GUARDRAIL)", () => {
    ...
    expect(p.metadata.name).toBe("mtf-trend-confluence-v1");
    expect(p.metadata.edgeClass).toBe("directional");
  });

  it("registry accepts the plugin", () => {
    const registry = new StrategyRegistry();
    registry.register(new MtfTrendConfluencePlugin());
  ...
  it("onBar emits a DirectionSignal (flat by default with minimal context)", () => {
    ...
    expect(sig.source).toBe("mtf-trend-confluence-v1");
  });
  ...
  it("construction rejects leverage ∉ {1, 10}", () => {
    expect(() => new MtfTrendConfluencePlugin({ leverage: 5 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
    expect(() => new MtfTrendConfluencePlugin({ leverage: 0 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
  });

  it("construction rejects non-positive baseNotionalUsd", () => {
    expect(() => new MtfTrendConfluencePlugin({ baseNotionalUsd: 0 })).toThrow(/baseNotionalUsd/);
    expect(() => new MtfTrendConfluencePlugin({ baseNotionalUsd: -1 })).toThrow(/baseNotionalUsd/);
  });
  ...
  it("effectiveMaxNotionalUsd is baseNotional × 10", () => {
    expect(new MtfTrendConfluencePlugin({ baseNotionalUsd: 5_000 }).effectiveMaxNotionalUsd()).toBe(50_000);
    expect(new MtfTrendConfluencePlugin().effectiveMaxNotionalUsd()).toBe(100_000);
  });

  it("createMtfTrendConfluencePlugin factory works", () => {
    const p = createMtfTrendConfluencePlugin();
    expect(p).toBeInstanceOf(MtfTrendConfluencePlugin);
  });

  it("DEFAULT_MTF_TREND_CONFLUENCE_PLUGIN_CONFIG has expected invariants", () => {
    expect(DEFAULT_MTF_TREND_CONFLUENCE_PLUGIN_CONFIG.leverage).toBe(10);
    expect(DEFAULT_MTF_TREND_CONFLUENCE_PLUGIN_CONFIG.baseNotionalUsd).toBe(10_000);
  });
  ...
  it("leverage=1 (baseline) accepted", () => {
    const p = new MtfTrendConfluencePlugin({ leverage: 1 });
