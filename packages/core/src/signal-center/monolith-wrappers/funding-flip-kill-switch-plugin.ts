// packages/core/src/signal-center/monolith-wrappers/funding-flip-kill-switch-plugin.ts
// — Phase 13 Track A
//
// ===========================================================================
// FundingFlipKillSwitchPlugin — wraps `FundingFlipKillSwitchStrategy` (funding-flip-kill-switch)
// ===========================================================================
//
// The wrapper:
//   - holds a `FundingFlipKillSwitchStrategy` instance
...
/**
 * `FundingFlipKillSwitchPluginConfig` — configuration for the wrapper. Includes the
 * underlying strategy's knobs plus SCv1 envelope settings
 * (baseNotionalUsd, leverage).
 */
export interface FundingFlipKillSwitchPluginConfig {
  ...
  readonly strategy?: Partial<FundingFlipKillSwitchConfig>;
  /** Leverage invariant config (Layer 2/3 reference). Default 1:10. */
  readonly leverageInvariant: LeverageInvariantConfig;
}

export const DEFAULT_FUNDING_FLIP_KILL_SWITCH_PLUGIN_CONFIG: Omit<
  FundingFlipKillSwitchPluginConfig,
  "strategy"
> = {
...
/**
 * `FundingFlipKillSwitchPluginState` — per-plugin mutable state held across `onBar` calls.
 */
export interface FundingFlipKillSwitchPluginState {
...
export class FundingFlipKillSwitchPlugin implements StrategyPlugin {
  readonly metadata: StrategyPluginMetadata = {
    name: "funding-flip-kill-switch-v1",
    version: "1.0.0",
    edgeClass: "risk",
    capitalRequirement: 10_000,
    maxLeverage: ONE_TO_TEN_LEVERAGE, // 1:10 HARD GUARDRAIL
    description:
      "Phase 13 Track A wrapper around FundingFlipKillSwitchStrategy. " +
  ...
  readonly config: FundingFlipKillSwitchPluginConfig;
  readonly state: FundingFlipKillSwitchPluginState;
  /** Underlying monolith strategy. */
  private readonly underlying: FundingFlipKillSwitchStrategy;
  ...
  constructor(config: Partial<FundingFlipKillSwitchPluginConfig> = {}) {
    const merged: FundingFlipKillSwitchPluginConfig = {
      ...DEFAULT_FUNDING_FLIP_KILL_SWITCH_PLUGIN_CONFIG,
      ...config,
    };
    // LAYER 1 — constructor check on metadata.maxLeverage.
    if (this.metadata.maxLeverage !== ONE_TO_TEN_LEVERAGE) {
      throw new Error(
        `[FundingFlipKillSwitchPlugin] 1:10 HARD GUARDRAIL VIOLATION: metadata.maxLeverage=${String(this.metadata.maxLeverage)} but the project-wide 1:10 mandate requires 10.`,
      );
    }
    // LAYER 1 — constructor check on leverage value.
    if (merged.leverage !== (1 as 1 | 10) && merged.leverage !== (10 as 1 | 10)) {
      throw new Error(
        `[FundingFlipKillSwitchPlugin] 1:10 HARD GUARDRAIL VIOLATION: leverage=${String(merged.leverage)} is NOT ALLOWED. Only 1 (baseline) or 10 (1:10 mandatory) are accepted.`,
      );
    }
    if (
      ...
      throw new Error(
        `[FundingFlipKillSwitchPlugin] baseNotionalUsd must be positive finite, got ${String(merged.baseNotionalUsd)}`,
      );
    }
    this.config = merged;
    this.underlying = new FundingFlipKillSwitchStrategy({ ...DEFAULT_FUNDING_FLIP_KILL_SWITCH_CONFIG, ...merged.strategy });
  ...
  subscribe(bus: SignalBus): void {
    ...
    try {
      ...
      throw new Error(
        `[FundingFlipKillSwitchPlugin] LAYER 2 BREACH on subscribe: ${msg}`,
        { cause: e },
      );
    }
  }
  ...
  validateConfig(config: unknown): Result<void, ConfigError> {
    ...
    const c = config as Partial<FundingFlipKillSwitchPluginConfig>;
    if (
  ...
  private _mkState(): FundingFlipKillSwitchPluginState {
    return {
  ...
  private _emitSizing(strength: number, timestampMs: number): void {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[FundingFlipKillSwitchPlugin] LAYER 3 BREACH on sizing emit: ${msg}`,
        { cause: e },
      );
    }
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[FundingFlipKillSwitchPlugin] LAYER 3 BREACH post-clamp: ${msg}`,
        { cause: e },
      );
    }
...
/**
 * `createFundingFlipKillSwitchPlugin` — convenience factory.
 */
export function createFundingFlipKillSwitchPlugin(
  config?: Partial<FundingFlipKillSwitchPluginConfig>,
): FundingFlipKillSwitchPlugin {
  return new FundingFlipKillSwitchPlugin(config);
}
