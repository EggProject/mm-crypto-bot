// packages/core/src/signal-center/monolith-wrappers/funding-flip-kill-switch-plugin.test.ts
// — Phase 13 Track A
//
// Test suite for FundingFlipKillSwitchPlugin — ≥5 tests covering:
...
import {
  ...
  createFundingFlipKillSwitchPlugin,
  DEFAULT_FUNDING_FLIP_KILL_SWITCH_PLUGIN_CONFIG,
} from "./funding-flip-kill-switch-plugin.js";
import type { Bar } from "../types.js";

const mkBus = (): SignalBus => new SignalBus({ mode: "backtest" });

const wirePlugin = (p: FundingFlipKillSwitchPlugin): SignalBus => {
  const bus = mkBus();
  p.subscribe(bus);
  return bus;
};
...
describe("FundingFlipKillSwitchPlugin", () => {
  it("construction with default config succeeds", () => {
    const p = new FundingFlipKillSwitchPlugin();
  ...
  it("metadata declares maxLeverage=10 (1:10 HARD GUARDRAIL)", () => {
    ...
    expect(p.metadata.name).toBe("funding-flip-kill-switch-v1");
    expect(p.metadata.edgeClass).toBe("risk");
  });

  it("registry accepts the plugin", () => {
    const registry = new StrategyRegistry();
    registry.register(new FundingFlipKillSwitchPlugin());
  ...
  it("onBar emits a DirectionSignal (flat by default with minimal context)", () => {
    ...
    expect(sig.source).toBe("funding-flip-kill-switch-v1");
  });
  ...
  it("construction rejects leverage ∉ {1, 10}", () => {
    expect(() => new FundingFlipKillSwitchPlugin({ leverage: 5 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
    expect(() => new FundingFlipKillSwitchPlugin({ leverage: 0 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
  });

  it("construction rejects non-positive baseNotionalUsd", () => {
    expect(() => new FundingFlipKillSwitchPlugin({ baseNotionalUsd: 0 })).toThrow(/baseNotionalUsd/);
    expect(() => new FundingFlipKillSwitchPlugin({ baseNotionalUsd: -1 })).toThrow(/baseNotionalUsd/);
  });
  ...
  it("effectiveMaxNotionalUsd is baseNotional × 10", () => {
    expect(new FundingFlipKillSwitchPlugin({ baseNotionalUsd: 5_000 }).effectiveMaxNotionalUsd()).toBe(50_000);
    expect(new FundingFlipKillSwitchPlugin().effectiveMaxNotionalUsd()).toBe(100_000);
  });

  it("createFundingFlipKillSwitchPlugin factory works", () => {
    const p = createFundingFlipKillSwitchPlugin();
    expect(p).toBeInstanceOf(FundingFlipKillSwitchPlugin);
  });

  it("DEFAULT_FUNDING_FLIP_KILL_SWITCH_PLUGIN_CONFIG has expected invariants", () => {
    expect(DEFAULT_FUNDING_FLIP_KILL_SWITCH_PLUGIN_CONFIG.leverage).toBe(10);
    expect(DEFAULT_FUNDING_FLIP_KILL_SWITCH_PLUGIN_CONFIG.baseNotionalUsd).toBe(10_000);
  });
  ...
  it("leverage=1 (baseline) accepted", () => {
    const p = new FundingFlipKillSwitchPlugin({ leverage: 1 });
