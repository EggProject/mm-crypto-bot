// packages/core/src/signal-center/monolith-wrappers/multi-class-ensemble-plugin.test.ts
// — Phase 13 Track A
//
// Test suite for MultiClassEnsemblePlugin — ≥5 tests covering:
...
import {
  ...
  createMultiClassEnsemblePlugin,
  DEFAULT_MULTI_CLASS_ENSEMBLE_PLUGIN_CONFIG,
} from "./multi-class-ensemble-plugin.js";
import type { Bar } from "../types.js";

const mkBus = (): SignalBus => new SignalBus({ mode: "backtest" });

const wirePlugin = (p: MultiClassEnsemblePlugin): SignalBus => {
  const bus = mkBus();
  p.subscribe(bus);
  return bus;
};
...
describe("MultiClassEnsemblePlugin", () => {
  it("construction with default config succeeds", () => {
    const p = new MultiClassEnsemblePlugin();
  ...
  it("metadata declares maxLeverage=10 (1:10 HARD GUARDRAIL)", () => {
    ...
    expect(p.metadata.name).toBe("multi-class-ensemble-v1");
    expect(p.metadata.edgeClass).toBe("mixed");
  });

  it("registry accepts the plugin", () => {
    const registry = new StrategyRegistry();
    registry.register(new MultiClassEnsemblePlugin());
  ...
  it("onBar emits a DirectionSignal (flat by default with minimal context)", () => {
    ...
    expect(sig.source).toBe("multi-class-ensemble-v1");
  });
  ...
  it("construction rejects leverage ∉ {1, 10}", () => {
    expect(() => new MultiClassEnsemblePlugin({ leverage: 5 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
    expect(() => new MultiClassEnsemblePlugin({ leverage: 0 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
  });

  it("construction rejects non-positive baseNotionalUsd", () => {
    expect(() => new MultiClassEnsemblePlugin({ baseNotionalUsd: 0 })).toThrow(/baseNotionalUsd/);
    expect(() => new MultiClassEnsemblePlugin({ baseNotionalUsd: -1 })).toThrow(/baseNotionalUsd/);
  });
  ...
  it("effectiveMaxNotionalUsd is baseNotional × 10", () => {
    expect(new MultiClassEnsemblePlugin({ baseNotionalUsd: 5_000 }).effectiveMaxNotionalUsd()).toBe(50_000);
    expect(new MultiClassEnsemblePlugin().effectiveMaxNotionalUsd()).toBe(100_000);
  });

  it("createMultiClassEnsemblePlugin factory works", () => {
    const p = createMultiClassEnsemblePlugin();
    expect(p).toBeInstanceOf(MultiClassEnsemblePlugin);
  });

  it("DEFAULT_MULTI_CLASS_ENSEMBLE_PLUGIN_CONFIG has expected invariants", () => {
    expect(DEFAULT_MULTI_CLASS_ENSEMBLE_PLUGIN_CONFIG.leverage).toBe(10);
    expect(DEFAULT_MULTI_CLASS_ENSEMBLE_PLUGIN_CONFIG.baseNotionalUsd).toBe(10_000);
  });
  ...
  it("leverage=1 (baseline) accepted", () => {
    const p = new MultiClassEnsemblePlugin({ leverage: 1 });
