// packages/core/src/signal-center/monolith-wrappers/funding-carry-timing-plugin.test.ts
// — Phase 13 Track A
//
// Test suite for FundingCarryTimingPlugin — ≥5 tests covering:
...
import {
  ...
  createFundingCarryTimingPlugin,
  DEFAULT_FUNDING_CARRY_TIMING_PLUGIN_CONFIG,
} from "./funding-carry-timing-plugin.js";
import type { Bar } from "../types.js";

const mkBus = (): SignalBus => new SignalBus({ mode: "backtest" });

const wirePlugin = (p: FundingCarryTimingPlugin): SignalBus => {
  const bus = mkBus();
  p.subscribe(bus);
  return bus;
};
...
describe("FundingCarryTimingPlugin", () => {
  it("construction with default config succeeds", () => {
    const p = new FundingCarryTimingPlugin();
  ...
  it("metadata declares maxLeverage=10 (1:10 HARD GUARDRAIL)", () => {
    ...
    expect(p.metadata.name).toBe("funding-carry-timing-v1");
    expect(p.metadata.edgeClass).toBe("carry");
  });

  it("registry accepts the plugin", () => {
    const registry = new StrategyRegistry();
    registry.register(new FundingCarryTimingPlugin());
  ...
  it("onBar emits a DirectionSignal (flat by default with minimal context)", () => {
    ...
    expect(sig.source).toBe("funding-carry-timing-v1");
  });
  ...
  it("construction rejects leverage ∉ {1, 10}", () => {
    expect(() => new FundingCarryTimingPlugin({ leverage: 5 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
    expect(() => new FundingCarryTimingPlugin({ leverage: 0 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
  });

  it("construction rejects non-positive baseNotionalUsd", () => {
    expect(() => new FundingCarryTimingPlugin({ baseNotionalUsd: 0 })).toThrow(/baseNotionalUsd/);
    expect(() => new FundingCarryTimingPlugin({ baseNotionalUsd: -1 })).toThrow(/baseNotionalUsd/);
  });
  ...
  it("effectiveMaxNotionalUsd is baseNotional × 10", () => {
    expect(new FundingCarryTimingPlugin({ baseNotionalUsd: 5_000 }).effectiveMaxNotionalUsd()).toBe(50_000);
    expect(new FundingCarryTimingPlugin().effectiveMaxNotionalUsd()).toBe(100_000);
  });

  it("createFundingCarryTimingPlugin factory works", () => {
    const p = createFundingCarryTimingPlugin();
    expect(p).toBeInstanceOf(FundingCarryTimingPlugin);
  });

  it("DEFAULT_FUNDING_CARRY_TIMING_PLUGIN_CONFIG has expected invariants", () => {
    expect(DEFAULT_FUNDING_CARRY_TIMING_PLUGIN_CONFIG.leverage).toBe(10);
    expect(DEFAULT_FUNDING_CARRY_TIMING_PLUGIN_CONFIG.baseNotionalUsd).toBe(10_000);
  });
  ...
  it("leverage=1 (baseline) accepted", () => {
    const p = new FundingCarryTimingPlugin({ leverage: 1 });
