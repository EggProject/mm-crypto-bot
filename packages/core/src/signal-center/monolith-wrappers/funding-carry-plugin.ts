// packages/core/src/signal-center/monolith-wrappers/funding-carry-plugin.ts
// — Phase 13 Track A
//
// ===========================================================================
// FundingCarryPlugin — wraps `FundingCarryStrategy` (funding-carry)
// ===========================================================================
//
// The wrapper:
//   - holds a `FundingCarryStrategy` instance
...
} from "../../strategy/funding-carry.js";
import {
  ONE_TO_TEN_LEVERAGE,
...
/**
 * `FundingCarryPluginConfig` — configuration for the wrapper. Includes the
 * underlying strategy's knobs plus SCv1 envelope settings
 * (baseNotionalUsd, leverage).
 */
export interface FundingCarryPluginConfig {
  ...
  readonly strategy?: Partial<FundingCarryConfig>;
  /** Leverage invariant config (Layer 2/3 reference). Default 1:10. */
  readonly leverageInvariant: LeverageInvariantConfig;
}

export const DEFAULT_FUNDING_CARRY_PLUGIN_CONFIG: Omit<
  FundingCarryPluginConfig,
  "strategy"
> = {
...
/**
 * `FundingCarryPluginState` — per-plugin mutable state held across `onBar` calls.
 */
export interface FundingCarryPluginState {
...
export class FundingCarryPlugin implements StrategyPlugin {
  readonly metadata: StrategyPluginMetadata = {
    ...
    description:
      "Phase 13 Track A wrapper around FundingCarryStrategy. " +
  ...
  readonly config: FundingCarryPluginConfig;
  readonly state: FundingCarryPluginState;
  /** Underlying monolith strategy. */
  private readonly underlying: FundingCarryStrategy;
  ...
  constructor(config: Partial<FundingCarryPluginConfig> = {}) {
    const merged: FundingCarryPluginConfig = {
      ...DEFAULT_FUNDING_CARRY_PLUGIN_CONFIG,
      ...config,
    };
    // LAYER 1 — constructor check on metadata.maxLeverage.
    if (this.metadata.maxLeverage !== ONE_TO_TEN_LEVERAGE) {
      throw new Error(
        `[FundingCarryPlugin] 1:10 HARD GUARDRAIL VIOLATION: metadata.maxLeverage=${String(this.metadata.maxLeverage)} but the project-wide 1:10 mandate requires 10.`,
      );
    }
    // LAYER 1 — constructor check on leverage value.
    if (merged.leverage !== (1 as 1 | 10) && merged.leverage !== (10 as 1 | 10)) {
      throw new Error(
        `[FundingCarryPlugin] 1:10 HARD GUARDRAIL VIOLATION: leverage=${String(merged.leverage)} is NOT ALLOWED. Only 1 (baseline) or 10 (1:10 mandatory) are accepted.`,
      );
    }
    if (
      ...
      throw new Error(
        `[FundingCarryPlugin] baseNotionalUsd must be positive finite, got ${String(merged.baseNotionalUsd)}`,
      );
    }
    this.config = merged;
    this.underlying = new FundingCarryStrategy({ ...DEFAULT_FUNDING_CARRY_CONFIG, ...merged.strategy });
  ...
  subscribe(bus: SignalBus): void {
    ...
    try {
      ...
      throw new Error(
        `[FundingCarryPlugin] LAYER 2 BREACH on subscribe: ${msg}`,
        { cause: e },
      );
    }
  }
  ...
  validateConfig(config: unknown): Result<void, ConfigError> {
    ...
    const c = config as Partial<FundingCarryPluginConfig>;
    if (
  ...
  private _mkState(): FundingCarryPluginState {
    return {
  ...
  private _emitSizing(strength: number, timestampMs: number): void {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[FundingCarryPlugin] LAYER 3 BREACH on sizing emit: ${msg}`,
        { cause: e },
      );
    }
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[FundingCarryPlugin] LAYER 3 BREACH post-clamp: ${msg}`,
        { cause: e },
      );
    }
...
/**
 * `createFundingCarryPlugin` — convenience factory.
 */
export function createFundingCarryPlugin(
  config?: Partial<FundingCarryPluginConfig>,
): FundingCarryPlugin {
  return new FundingCarryPlugin(config);
}
