// packages/core/src/signal-center/monolith-wrappers/funding-carry-leverage-plugin.ts
// — Phase 13 Track A
//
// ===========================================================================
// FundingCarryLeveragePlugin — wraps `FundingCarryLeverageStrategy` (funding-carry-leverage)
// ===========================================================================
//
// The wrapper:
//   - holds a `FundingCarryLeverageStrategy` instance
...
/**
 * `FundingCarryLeveragePluginConfig` — configuration for the wrapper. Includes the
 * underlying strategy's knobs plus SCv1 envelope settings
 * (baseNotionalUsd, leverage).
 */
export interface FundingCarryLeveragePluginConfig {
  ...
  readonly strategy?: Partial<LeveragedCarryConfig>;
  /** Leverage invariant config (Layer 2/3 reference). Default 1:10. */
  readonly leverageInvariant: LeverageInvariantConfig;
}

export const DEFAULT_FUNDING_CARRY_LEVERAGE_PLUGIN_CONFIG: Omit<
  FundingCarryLeveragePluginConfig,
  "strategy"
> = {
...
/**
 * `FundingCarryLeveragePluginState` — per-plugin mutable state held across `onBar` calls.
 */
export interface FundingCarryLeveragePluginState {
...
export class FundingCarryLeveragePlugin implements StrategyPlugin {
  readonly metadata: StrategyPluginMetadata = {
    name: "funding-carry-leverage-v1",
    version: "1.0.0",
    edgeClass: "carry",
    capitalRequirement: 10_000,
    maxLeverage: ONE_TO_TEN_LEVERAGE, // 1:10 HARD GUARDRAIL
    description:
      "Phase 13 Track A wrapper around FundingCarryLeverageStrategy. " +
  ...
  readonly config: FundingCarryLeveragePluginConfig;
  readonly state: FundingCarryLeveragePluginState;
  /** Underlying monolith strategy. */
  private readonly underlying: FundingCarryLeverageStrategy;
  ...
  constructor(config: Partial<FundingCarryLeveragePluginConfig> = {}) {
    const merged: FundingCarryLeveragePluginConfig = {
      ...DEFAULT_FUNDING_CARRY_LEVERAGE_PLUGIN_CONFIG,
      ...config,
    };
    // LAYER 1 — constructor check on metadata.maxLeverage.
    if (this.metadata.maxLeverage !== ONE_TO_TEN_LEVERAGE) {
      throw new Error(
        `[FundingCarryLeveragePlugin] 1:10 HARD GUARDRAIL VIOLATION: metadata.maxLeverage=${String(this.metadata.maxLeverage)} but the project-wide 1:10 mandate requires 10.`,
      );
    }
    // LAYER 1 — constructor check on leverage value.
    if (merged.leverage !== (1 as 1 | 10) && merged.leverage !== (10 as 1 | 10)) {
      throw new Error(
        `[FundingCarryLeveragePlugin] 1:10 HARD GUARDRAIL VIOLATION: leverage=${String(merged.leverage)} is NOT ALLOWED. Only 1 (baseline) or 10 (1:10 mandatory) are accepted.`,
      );
    }
    if (
      ...
      throw new Error(
        `[FundingCarryLeveragePlugin] baseNotionalUsd must be positive finite, got ${String(merged.baseNotionalUsd)}`,
      );
    }
    this.config = merged;
    this.underlying = new FundingCarryLeverageStrategy({ ...DEFAULT_LEVERAGED_CARRY_CONFIG, ...merged.strategy });
  ...
  subscribe(bus: SignalBus): void {
    ...
    try {
      ...
      throw new Error(
        `[FundingCarryLeveragePlugin] LAYER 2 BREACH on subscribe: ${msg}`,
        { cause: e },
      );
    }
  }
  ...
  validateConfig(config: unknown): Result<void, ConfigError> {
    ...
    const c = config as Partial<FundingCarryLeveragePluginConfig>;
    if (
  ...
  private _mkState(): FundingCarryLeveragePluginState {
    return {
  ...
  private _emitSizing(strength: number, timestampMs: number): void {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[FundingCarryLeveragePlugin] LAYER 3 BREACH on sizing emit: ${msg}`,
        { cause: e },
      );
    }
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[FundingCarryLeveragePlugin] LAYER 3 BREACH post-clamp: ${msg}`,
        { cause: e },
      );
    }
...
/**
 * `createFundingCarryLeveragePlugin` — convenience factory.
 */
export function createFundingCarryLeveragePlugin(
  config?: Partial<FundingCarryLeveragePluginConfig>,
): FundingCarryLeveragePlugin {
  return new FundingCarryLeveragePlugin(config);
}
