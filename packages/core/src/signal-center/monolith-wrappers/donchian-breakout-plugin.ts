// packages/core/src/signal-center/monolith-wrappers/donchian-breakout-plugin.ts
// — Phase 13 Track A
//
// ===========================================================================
// DonchianBreakoutPlugin — wraps `DonchianBreakoutStrategy` (donchian-breakout)
// ===========================================================================
//
// The wrapper:
//   - holds a `DonchianBreakoutStrategy` instance
...
} from "../../strategy/donchian-breakout.js";
import {
  ONE_TO_TEN_LEVERAGE,
...
/**
 * `DonchianBreakoutPluginConfig` — configuration for the wrapper. Includes the
 * underlying strategy's knobs plus SCv1 envelope settings
 * (baseNotionalUsd, leverage).
 */
export interface DonchianBreakoutPluginConfig {
  ...
  readonly strategy?: Partial<DonchianBreakoutConfig>;
  /** Leverage invariant config (Layer 2/3 reference). Default 1:10. */
  readonly leverageInvariant: LeverageInvariantConfig;
}

export const DEFAULT_DONCHIAN_BREAKOUT_PLUGIN_CONFIG: Omit<
  DonchianBreakoutPluginConfig,
  "strategy"
> = {
...
/**
 * `DonchianBreakoutPluginState` — per-plugin mutable state held across `onBar` calls.
 */
export interface DonchianBreakoutPluginState {
...
export class DonchianBreakoutPlugin implements StrategyPlugin {
  readonly metadata: StrategyPluginMetadata = {
    ...
    description:
      "Phase 13 Track A wrapper around DonchianBreakoutStrategy. " +
  ...
  readonly config: DonchianBreakoutPluginConfig;
  readonly state: DonchianBreakoutPluginState;
  /** Underlying monolith strategy. */
  private readonly underlying: DonchianBreakoutStrategy;
  ...
  constructor(config: Partial<DonchianBreakoutPluginConfig> = {}) {
    const merged: DonchianBreakoutPluginConfig = {
      ...DEFAULT_DONCHIAN_BREAKOUT_PLUGIN_CONFIG,
      ...config,
    };
    // LAYER 1 — constructor check on metadata.maxLeverage.
    if (this.metadata.maxLeverage !== ONE_TO_TEN_LEVERAGE) {
      throw new Error(
        `[DonchianBreakoutPlugin] 1:10 HARD GUARDRAIL VIOLATION: metadata.maxLeverage=${String(this.metadata.maxLeverage)} but the project-wide 1:10 mandate requires 10.`,
      );
    }
    // LAYER 1 — constructor check on leverage value.
    if (merged.leverage !== (1 as 1 | 10) && merged.leverage !== (10 as 1 | 10)) {
      throw new Error(
        `[DonchianBreakoutPlugin] 1:10 HARD GUARDRAIL VIOLATION: leverage=${String(merged.leverage)} is NOT ALLOWED. Only 1 (baseline) or 10 (1:10 mandatory) are accepted.`,
      );
    }
    if (
      ...
      throw new Error(
        `[DonchianBreakoutPlugin] baseNotionalUsd must be positive finite, got ${String(merged.baseNotionalUsd)}`,
      );
    }
    this.config = merged;
    this.underlying = new DonchianBreakoutStrategy({ ...DEFAULT_DONCHIAN_CONFIG, ...merged.strategy });
  ...
  subscribe(bus: SignalBus): void {
    ...
    try {
      ...
      throw new Error(
        `[DonchianBreakoutPlugin] LAYER 2 BREACH on subscribe: ${msg}`,
        { cause: e },
      );
    }
  }
  ...
  validateConfig(config: unknown): Result<void, ConfigError> {
    ...
    const c = config as Partial<DonchianBreakoutPluginConfig>;
    if (
  ...
  private _mkState(): DonchianBreakoutPluginState {
    return {
  ...
  private _emitSizing(strength: number, timestampMs: number): void {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[DonchianBreakoutPlugin] LAYER 3 BREACH on sizing emit: ${msg}`,
        { cause: e },
      );
    }
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[DonchianBreakoutPlugin] LAYER 3 BREACH post-clamp: ${msg}`,
        { cause: e },
      );
    }
...
/**
 * `createDonchianBreakoutPlugin` — convenience factory.
 */
export function createDonchianBreakoutPlugin(
  config?: Partial<DonchianBreakoutPluginConfig>,
): DonchianBreakoutPlugin {
  return new DonchianBreakoutPlugin(config);
}
