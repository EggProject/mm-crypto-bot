// packages/core/src/signal-center/monolith-wrappers/donchian-trailing-plugin.test.ts
// — Phase 13 Track A
//
// Test suite for DonchianTrailingPlugin — ≥5 tests covering:
...
import {
  ...
  createDonchianTrailingPlugin,
  DEFAULT_DONCHIAN_TRAILING_PLUGIN_CONFIG,
} from "./donchian-trailing-plugin.js";
import type { Bar } from "../types.js";

const mkBus = (): SignalBus => new SignalBus({ mode: "backtest" });

const wirePlugin = (p: DonchianTrailingPlugin): SignalBus => {
  const bus = mkBus();
  p.subscribe(bus);
  return bus;
};
...
describe("DonchianTrailingPlugin", () => {
  it("construction with default config succeeds", () => {
    const p = new DonchianTrailingPlugin();
  ...
  it("metadata declares maxLeverage=10 (1:10 HARD GUARDRAIL)", () => {
    ...
    expect(p.metadata.name).toBe("donchian-trailing-v1");
    expect(p.metadata.edgeClass).toBe("directional");
  });

  it("registry accepts the plugin", () => {
    const registry = new StrategyRegistry();
    registry.register(new DonchianTrailingPlugin());
  ...
  it("onBar emits a DirectionSignal (flat by default with minimal context)", () => {
    ...
    expect(sig.source).toBe("donchian-trailing-v1");
  });
  ...
  it("construction rejects leverage ∉ {1, 10}", () => {
    expect(() => new DonchianTrailingPlugin({ leverage: 5 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
    expect(() => new DonchianTrailingPlugin({ leverage: 0 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
  });

  it("construction rejects non-positive baseNotionalUsd", () => {
    expect(() => new DonchianTrailingPlugin({ baseNotionalUsd: 0 })).toThrow(/baseNotionalUsd/);
    expect(() => new DonchianTrailingPlugin({ baseNotionalUsd: -1 })).toThrow(/baseNotionalUsd/);
  });
  ...
  it("effectiveMaxNotionalUsd is baseNotional × 10", () => {
    expect(new DonchianTrailingPlugin({ baseNotionalUsd: 5_000 }).effectiveMaxNotionalUsd()).toBe(50_000);
    expect(new DonchianTrailingPlugin().effectiveMaxNotionalUsd()).toBe(100_000);
  });

  it("createDonchianTrailingPlugin factory works", () => {
    const p = createDonchianTrailingPlugin();
    expect(p).toBeInstanceOf(DonchianTrailingPlugin);
  });

  it("DEFAULT_DONCHIAN_TRAILING_PLUGIN_CONFIG has expected invariants", () => {
    expect(DEFAULT_DONCHIAN_TRAILING_PLUGIN_CONFIG.leverage).toBe(10);
    expect(DEFAULT_DONCHIAN_TRAILING_PLUGIN_CONFIG.baseNotionalUsd).toBe(10_000);
  });
  ...
  it("leverage=1 (baseline) accepted", () => {
    const p = new DonchianTrailingPlugin({ leverage: 1 });
