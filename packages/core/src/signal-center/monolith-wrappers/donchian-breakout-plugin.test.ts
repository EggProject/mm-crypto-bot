// packages/core/src/signal-center/monolith-wrappers/donchian-breakout-plugin.test.ts
// — Phase 13 Track A
//
// Test suite for DonchianBreakoutPlugin — ≥5 tests covering:
...
import {
  ...
  createDonchianBreakoutPlugin,
  DEFAULT_DONCHIAN_BREAKOUT_PLUGIN_CONFIG,
} from "./donchian-breakout-plugin.js";
import type { Bar } from "../types.js";

const mkBus = (): SignalBus => new SignalBus({ mode: "backtest" });

const wirePlugin = (p: DonchianBreakoutPlugin): SignalBus => {
  const bus = mkBus();
  p.subscribe(bus);
  return bus;
};
...
describe("DonchianBreakoutPlugin", () => {
  it("construction with default config succeeds", () => {
    const p = new DonchianBreakoutPlugin();
  ...
  it("metadata declares maxLeverage=10 (1:10 HARD GUARDRAIL)", () => {
    ...
    expect(p.metadata.name).toBe("donchian-breakout-v1");
    expect(p.metadata.edgeClass).toBe("directional");
  });

  it("registry accepts the plugin", () => {
    const registry = new StrategyRegistry();
    registry.register(new DonchianBreakoutPlugin());
  ...
  it("onBar emits a DirectionSignal (flat by default with minimal context)", () => {
    ...
    expect(sig.source).toBe("donchian-breakout-v1");
  });
  ...
  it("construction rejects leverage ∉ {1, 10}", () => {
    expect(() => new DonchianBreakoutPlugin({ leverage: 5 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
    expect(() => new DonchianBreakoutPlugin({ leverage: 0 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
  });

  it("construction rejects non-positive baseNotionalUsd", () => {
    expect(() => new DonchianBreakoutPlugin({ baseNotionalUsd: 0 })).toThrow(/baseNotionalUsd/);
    expect(() => new DonchianBreakoutPlugin({ baseNotionalUsd: -1 })).toThrow(/baseNotionalUsd/);
  });
  ...
  it("effectiveMaxNotionalUsd is baseNotional × 10", () => {
    expect(new DonchianBreakoutPlugin({ baseNotionalUsd: 5_000 }).effectiveMaxNotionalUsd()).toBe(50_000);
    expect(new DonchianBreakoutPlugin().effectiveMaxNotionalUsd()).toBe(100_000);
  });

  it("createDonchianBreakoutPlugin factory works", () => {
    const p = createDonchianBreakoutPlugin();
    expect(p).toBeInstanceOf(DonchianBreakoutPlugin);
  });

  it("DEFAULT_DONCHIAN_BREAKOUT_PLUGIN_CONFIG has expected invariants", () => {
    expect(DEFAULT_DONCHIAN_BREAKOUT_PLUGIN_CONFIG.leverage).toBe(10);
    expect(DEFAULT_DONCHIAN_BREAKOUT_PLUGIN_CONFIG.baseNotionalUsd).toBe(10_000);
  });
  ...
  it("leverage=1 (baseline) accepted", () => {
    const p = new DonchianBreakoutPlugin({ leverage: 1 });
