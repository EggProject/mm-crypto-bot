// packages/core/src/signal-center/monolith-wrappers/mean-reversion-bb-plugin.test.ts
// — Phase 13 Track A
//
// Test suite for MeanReversionBbPlugin — ≥5 tests covering:
...
import {
  ...
  createMeanReversionBbPlugin,
  DEFAULT_MEAN_REVERSION_BB_PLUGIN_CONFIG,
} from "./mean-reversion-bb-plugin.js";
import type { Bar } from "../types.js";

const mkBus = (): SignalBus => new SignalBus({ mode: "backtest" });

const wirePlugin = (p: MeanReversionBbPlugin): SignalBus => {
  const bus = mkBus();
  p.subscribe(bus);
  return bus;
};
...
describe("MeanReversionBbPlugin", () => {
  it("construction with default config succeeds", () => {
    const p = new MeanReversionBbPlugin();
  ...
  it("metadata declares maxLeverage=10 (1:10 HARD GUARDRAIL)", () => {
    ...
    expect(p.metadata.name).toBe("mean-reversion-bb-v1");
    expect(p.metadata.edgeClass).toBe("directional");
  });

  it("registry accepts the plugin", () => {
    const registry = new StrategyRegistry();
    registry.register(new MeanReversionBbPlugin());
  ...
  it("onBar emits a DirectionSignal (flat by default with minimal context)", () => {
    ...
    expect(sig.source).toBe("mean-reversion-bb-v1");
  });
  ...
  it("construction rejects leverage ∉ {1, 10}", () => {
    expect(() => new MeanReversionBbPlugin({ leverage: 5 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
    expect(() => new MeanReversionBbPlugin({ leverage: 0 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
  });

  it("construction rejects non-positive baseNotionalUsd", () => {
    expect(() => new MeanReversionBbPlugin({ baseNotionalUsd: 0 })).toThrow(/baseNotionalUsd/);
    expect(() => new MeanReversionBbPlugin({ baseNotionalUsd: -1 })).toThrow(/baseNotionalUsd/);
  });
  ...
  it("effectiveMaxNotionalUsd is baseNotional × 10", () => {
    expect(new MeanReversionBbPlugin({ baseNotionalUsd: 5_000 }).effectiveMaxNotionalUsd()).toBe(50_000);
    expect(new MeanReversionBbPlugin().effectiveMaxNotionalUsd()).toBe(100_000);
  });

  it("createMeanReversionBbPlugin factory works", () => {
    const p = createMeanReversionBbPlugin();
    expect(p).toBeInstanceOf(MeanReversionBbPlugin);
  });

  it("DEFAULT_MEAN_REVERSION_BB_PLUGIN_CONFIG has expected invariants", () => {
    expect(DEFAULT_MEAN_REVERSION_BB_PLUGIN_CONFIG.leverage).toBe(10);
    expect(DEFAULT_MEAN_REVERSION_BB_PLUGIN_CONFIG.baseNotionalUsd).toBe(10_000);
  });
  ...
  it("leverage=1 (baseline) accepted", () => {
    const p = new MeanReversionBbPlugin({ leverage: 1 });
