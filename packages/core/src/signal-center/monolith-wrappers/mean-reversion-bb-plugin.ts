// packages/core/src/signal-center/monolith-wrappers/mean-reversion-bb-plugin.ts
// — Phase 13 Track A
//
// ===========================================================================
// MeanReversionBbPlugin — wraps `MeanReversionBbStrategy` (mean-reversion-bb)
// ===========================================================================
//
// The wrapper:
//   - holds a `MeanReversionBbStrategy` instance
...
import {
  ...
  type MeanReversionBbConfig,
  DEFAULT_MR_CONFIG,
} from "../../strategy/mean-reversion-bb.js";
import {
  ONE_TO_TEN_LEVERAGE,
...
/**
 * `MeanReversionBbPluginConfig` — configuration for the wrapper. Includes the
 * underlying strategy's knobs plus SCv1 envelope settings
 * (baseNotionalUsd, leverage).
 */
export interface MeanReversionBbPluginConfig {
  ...
  readonly strategy?: Partial<MeanReversionBbConfig>;
  /** Leverage invariant config (Layer 2/3 reference). Default 1:10. */
  readonly leverageInvariant: LeverageInvariantConfig;
}

export const DEFAULT_MEAN_REVERSION_BB_PLUGIN_CONFIG: Omit<
  MeanReversionBbPluginConfig,
  "strategy"
> = {
...
/**
 * `MeanReversionBbPluginState` — per-plugin mutable state held across `onBar` calls.
 */
export interface MeanReversionBbPluginState {
...
export class MeanReversionBbPlugin implements StrategyPlugin {
  readonly metadata: StrategyPluginMetadata = {
    name: "mean-reversion-bb-v1",
    version: "1.0.0",
    edgeClass: "directional",
    capitalRequirement: 10_000,
    maxLeverage: ONE_TO_TEN_LEVERAGE, // 1:10 HARD GUARDRAIL
    description:
      "Phase 13 Track A wrapper around MeanReversionBbStrategy. " +
  ...
  readonly config: MeanReversionBbPluginConfig;
  readonly state: MeanReversionBbPluginState;
  /** Underlying monolith strategy. */
  private readonly underlying: MeanReversionBbStrategy;
  ...
  constructor(config: Partial<MeanReversionBbPluginConfig> = {}) {
    const merged: MeanReversionBbPluginConfig = {
      ...DEFAULT_MEAN_REVERSION_BB_PLUGIN_CONFIG,
      ...config,
    };
    // LAYER 1 — constructor check on metadata.maxLeverage.
    if (this.metadata.maxLeverage !== ONE_TO_TEN_LEVERAGE) {
      throw new Error(
        `[MeanReversionBbPlugin] 1:10 HARD GUARDRAIL VIOLATION: metadata.maxLeverage=${String(this.metadata.maxLeverage)} but the project-wide 1:10 mandate requires 10.`,
      );
    }
    // LAYER 1 — constructor check on leverage value.
    if (merged.leverage !== (1 as 1 | 10) && merged.leverage !== (10 as 1 | 10)) {
      throw new Error(
        `[MeanReversionBbPlugin] 1:10 HARD GUARDRAIL VIOLATION: leverage=${String(merged.leverage)} is NOT ALLOWED. Only 1 (baseline) or 10 (1:10 mandatory) are accepted.`,
      );
    }
    if (
      ...
      throw new Error(
        `[MeanReversionBbPlugin] baseNotionalUsd must be positive finite, got ${String(merged.baseNotionalUsd)}`,
      );
    }
    this.config = merged;
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    this.underlying = new MeanReversionBbStrategy({ ...DEFAULT_MR_CONFIG, ...merged.strategy } as unknown as MeanReversionBbConfig);
  ...
  subscribe(bus: SignalBus): void {
    ...
    try {
      ...
      throw new Error(
        `[MeanReversionBbPlugin] LAYER 2 BREACH on subscribe: ${msg}`,
        { cause: e },
      );
    }
  }
  ...
  validateConfig(config: unknown): Result<void, ConfigError> {
    ...
    const c = config as Partial<MeanReversionBbPluginConfig>;
    if (
  ...
  private _mkState(): MeanReversionBbPluginState {
    return {
  ...
  private _emitSizing(strength: number, timestampMs: number): void {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[MeanReversionBbPlugin] LAYER 3 BREACH on sizing emit: ${msg}`,
        { cause: e },
      );
    }
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[MeanReversionBbPlugin] LAYER 3 BREACH post-clamp: ${msg}`,
        { cause: e },
      );
    }
...
/**
 * `createMeanReversionBbPlugin` — convenience factory.
 */
export function createMeanReversionBbPlugin(
  config?: Partial<MeanReversionBbPluginConfig>,
): MeanReversionBbPlugin {
  return new MeanReversionBbPlugin(config);
}
