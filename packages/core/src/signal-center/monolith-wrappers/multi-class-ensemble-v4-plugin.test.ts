// packages/core/src/signal-center/monolith-wrappers/multi-class-ensemble-v4-plugin.test.ts
// — Phase 13 Track A
//
// Test suite for MultiClassEnsembleV4Plugin — ≥5 tests covering:
...
import {
  ...
  createMultiClassEnsembleV4Plugin,
  DEFAULT_MULTI_CLASS_ENSEMBLE_V4_PLUGIN_CONFIG,
} from "./multi-class-ensemble-v4-plugin.js";
import type { Bar } from "../types.js";

const mkBus = (): SignalBus => new SignalBus({ mode: "backtest" });

const wirePlugin = (p: MultiClassEnsembleV4Plugin): SignalBus => {
  const bus = mkBus();
  p.subscribe(bus);
  return bus;
};
...
describe("MultiClassEnsembleV4Plugin", () => {
  it("construction with default config succeeds", () => {
    const p = new MultiClassEnsembleV4Plugin();
  ...
  it("metadata declares maxLeverage=10 (1:10 HARD GUARDRAIL)", () => {
    ...
    expect(p.metadata.name).toBe("multi-class-ensemble-v4-v1");
    expect(p.metadata.edgeClass).toBe("mixed");
  });

  it("registry accepts the plugin", () => {
    const registry = new StrategyRegistry();
    registry.register(new MultiClassEnsembleV4Plugin());
  ...
  it("onBar emits a DirectionSignal (flat by default with minimal context)", () => {
    ...
    expect(sig.source).toBe("multi-class-ensemble-v4-v1");
  });
  ...
  it("construction rejects leverage ∉ {1, 10}", () => {
    expect(() => new MultiClassEnsembleV4Plugin({ leverage: 5 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
    expect(() => new MultiClassEnsembleV4Plugin({ leverage: 0 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
  });

  it("construction rejects non-positive baseNotionalUsd", () => {
    expect(() => new MultiClassEnsembleV4Plugin({ baseNotionalUsd: 0 })).toThrow(/baseNotionalUsd/);
    expect(() => new MultiClassEnsembleV4Plugin({ baseNotionalUsd: -1 })).toThrow(/baseNotionalUsd/);
  });
  ...
  it("effectiveMaxNotionalUsd is baseNotional × 10", () => {
    expect(new MultiClassEnsembleV4Plugin({ baseNotionalUsd: 5_000 }).effectiveMaxNotionalUsd()).toBe(50_000);
    expect(new MultiClassEnsembleV4Plugin().effectiveMaxNotionalUsd()).toBe(100_000);
  });

  it("createMultiClassEnsembleV4Plugin factory works", () => {
    const p = createMultiClassEnsembleV4Plugin();
    expect(p).toBeInstanceOf(MultiClassEnsembleV4Plugin);
  });

  it("DEFAULT_MULTI_CLASS_ENSEMBLE_V4_PLUGIN_CONFIG has expected invariants", () => {
    expect(DEFAULT_MULTI_CLASS_ENSEMBLE_V4_PLUGIN_CONFIG.leverage).toBe(10);
    expect(DEFAULT_MULTI_CLASS_ENSEMBLE_V4_PLUGIN_CONFIG.baseNotionalUsd).toBe(10_000);
  });
  ...
  it("leverage=1 (baseline) accepted", () => {
    const p = new MultiClassEnsembleV4Plugin({ leverage: 1 });
