// packages/core/src/signal-center/monolith-wrappers/funding-carry-leverage-plugin.test.ts
// — Phase 13 Track A
//
// Test suite for FundingCarryLeveragePlugin — ≥5 tests covering:
...
import {
  ...
  createFundingCarryLeveragePlugin,
  DEFAULT_FUNDING_CARRY_LEVERAGE_PLUGIN_CONFIG,
} from "./funding-carry-leverage-plugin.js";
import type { Bar } from "../types.js";

const mkBus = (): SignalBus => new SignalBus({ mode: "backtest" });

const wirePlugin = (p: FundingCarryLeveragePlugin): SignalBus => {
  const bus = mkBus();
  p.subscribe(bus);
  return bus;
};
...
describe("FundingCarryLeveragePlugin", () => {
  it("construction with default config succeeds", () => {
    const p = new FundingCarryLeveragePlugin();
  ...
  it("metadata declares maxLeverage=10 (1:10 HARD GUARDRAIL)", () => {
    ...
    expect(p.metadata.name).toBe("funding-carry-leverage-v1");
    expect(p.metadata.edgeClass).toBe("carry");
  });

  it("registry accepts the plugin", () => {
    const registry = new StrategyRegistry();
    registry.register(new FundingCarryLeveragePlugin());
  ...
  it("onBar emits a DirectionSignal (flat by default with minimal context)", () => {
    ...
    expect(sig.source).toBe("funding-carry-leverage-v1");
  });
  ...
  it("construction rejects leverage ∉ {1, 10}", () => {
    expect(() => new FundingCarryLeveragePlugin({ leverage: 5 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
    expect(() => new FundingCarryLeveragePlugin({ leverage: 0 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
  });

  it("construction rejects non-positive baseNotionalUsd", () => {
    expect(() => new FundingCarryLeveragePlugin({ baseNotionalUsd: 0 })).toThrow(/baseNotionalUsd/);
    expect(() => new FundingCarryLeveragePlugin({ baseNotionalUsd: -1 })).toThrow(/baseNotionalUsd/);
  });
  ...
  it("effectiveMaxNotionalUsd is baseNotional × 10", () => {
    expect(new FundingCarryLeveragePlugin({ baseNotionalUsd: 5_000 }).effectiveMaxNotionalUsd()).toBe(50_000);
    expect(new FundingCarryLeveragePlugin().effectiveMaxNotionalUsd()).toBe(100_000);
  });

  it("createFundingCarryLeveragePlugin factory works", () => {
    const p = createFundingCarryLeveragePlugin();
    expect(p).toBeInstanceOf(FundingCarryLeveragePlugin);
  });

  it("DEFAULT_FUNDING_CARRY_LEVERAGE_PLUGIN_CONFIG has expected invariants", () => {
    expect(DEFAULT_FUNDING_CARRY_LEVERAGE_PLUGIN_CONFIG.leverage).toBe(10);
    expect(DEFAULT_FUNDING_CARRY_LEVERAGE_PLUGIN_CONFIG.baseNotionalUsd).toBe(10_000);
  });
  ...
  it("leverage=1 (baseline) accepted", () => {
    const p = new FundingCarryLeveragePlugin({ leverage: 1 });
