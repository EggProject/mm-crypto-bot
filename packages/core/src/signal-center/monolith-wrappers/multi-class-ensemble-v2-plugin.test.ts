// packages/core/src/signal-center/monolith-wrappers/multi-class-ensemble-v2-plugin.test.ts
// — Phase 13 Track A
//
// Test suite for MultiClassEnsembleV2Plugin — ≥5 tests covering:
...
import {
  ...
  createMultiClassEnsembleV2Plugin,
  DEFAULT_MULTI_CLASS_ENSEMBLE_V2_PLUGIN_CONFIG,
} from "./multi-class-ensemble-v2-plugin.js";
import type { Bar } from "../types.js";

const mkBus = (): SignalBus => new SignalBus({ mode: "backtest" });

const wirePlugin = (p: MultiClassEnsembleV2Plugin): SignalBus => {
  const bus = mkBus();
  p.subscribe(bus);
  return bus;
};
...
describe("MultiClassEnsembleV2Plugin", () => {
  it("construction with default config succeeds", () => {
    const p = new MultiClassEnsembleV2Plugin();
  ...
  it("metadata declares maxLeverage=10 (1:10 HARD GUARDRAIL)", () => {
    ...
    expect(p.metadata.name).toBe("multi-class-ensemble-v2-v1");
    expect(p.metadata.edgeClass).toBe("mixed");
  });

  it("registry accepts the plugin", () => {
    const registry = new StrategyRegistry();
    registry.register(new MultiClassEnsembleV2Plugin());
  ...
  it("onBar emits a DirectionSignal (flat by default with minimal context)", () => {
    ...
    expect(sig.source).toBe("multi-class-ensemble-v2-v1");
  });
  ...
  it("construction rejects leverage ∉ {1, 10}", () => {
    expect(() => new MultiClassEnsembleV2Plugin({ leverage: 5 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
    expect(() => new MultiClassEnsembleV2Plugin({ leverage: 0 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
  });

  it("construction rejects non-positive baseNotionalUsd", () => {
    expect(() => new MultiClassEnsembleV2Plugin({ baseNotionalUsd: 0 })).toThrow(/baseNotionalUsd/);
    expect(() => new MultiClassEnsembleV2Plugin({ baseNotionalUsd: -1 })).toThrow(/baseNotionalUsd/);
  });
  ...
  it("effectiveMaxNotionalUsd is baseNotional × 10", () => {
    expect(new MultiClassEnsembleV2Plugin({ baseNotionalUsd: 5_000 }).effectiveMaxNotionalUsd()).toBe(50_000);
    expect(new MultiClassEnsembleV2Plugin().effectiveMaxNotionalUsd()).toBe(100_000);
  });

  it("createMultiClassEnsembleV2Plugin factory works", () => {
    const p = createMultiClassEnsembleV2Plugin();
    expect(p).toBeInstanceOf(MultiClassEnsembleV2Plugin);
  });

  it("DEFAULT_MULTI_CLASS_ENSEMBLE_V2_PLUGIN_CONFIG has expected invariants", () => {
    expect(DEFAULT_MULTI_CLASS_ENSEMBLE_V2_PLUGIN_CONFIG.leverage).toBe(10);
    expect(DEFAULT_MULTI_CLASS_ENSEMBLE_V2_PLUGIN_CONFIG.baseNotionalUsd).toBe(10_000);
  });
  ...
  it("leverage=1 (baseline) accepted", () => {
    const p = new MultiClassEnsembleV2Plugin({ leverage: 1 });
