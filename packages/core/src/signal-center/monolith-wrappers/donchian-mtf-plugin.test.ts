// packages/core/src/signal-center/monolith-wrappers/donchian-mtf-plugin.test.ts
// — Phase 13 Track A
//
// Test suite for DonchianMtfPlugin — ≥5 tests covering:
...
import {
  DonchianMtfPlugin,
  createDonchianMtfPlugin,
  DEFAULT_DONCHIAN_MTF_PLUGIN_CONFIG,
} from "./donchian-mtf-plugin.js";
import type { Bar } from "../types.js";

const mkBus = (): SignalBus => new SignalBus({ mode: "backtest" });

const wirePlugin = (p: DonchianMtfPlugin): SignalBus => {
  const bus = mkBus();
  p.subscribe(bus);
  return bus;
};
...
describe("DonchianMtfPlugin", () => {
  it("construction with default config succeeds", () => {
    const p = new DonchianMtfPlugin();
  ...
  it("metadata declares maxLeverage=10 (1:10 HARD GUARDRAIL)", () => {
    ...
    expect(p.metadata.name).toBe("donchian-mtf-v1");
    expect(p.metadata.edgeClass).toBe("directional");
  });

  it("registry accepts the plugin", () => {
    const registry = new StrategyRegistry();
    registry.register(new DonchianMtfPlugin());
  ...
  it("onBar emits a DirectionSignal (flat by default with minimal context)", () => {
    ...
    expect(sig.source).toBe("donchian-mtf-v1");
  });
  ...
  it("construction rejects leverage ∉ {1, 10}", () => {
    expect(() => new DonchianMtfPlugin({ leverage: 5 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
    expect(() => new DonchianMtfPlugin({ leverage: 0 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
  });

  it("construction rejects non-positive baseNotionalUsd", () => {
    expect(() => new DonchianMtfPlugin({ baseNotionalUsd: 0 })).toThrow(/baseNotionalUsd/);
    expect(() => new DonchianMtfPlugin({ baseNotionalUsd: -1 })).toThrow(/baseNotionalUsd/);
  });
  ...
  it("effectiveMaxNotionalUsd is baseNotional × 10", () => {
    expect(new DonchianMtfPlugin({ baseNotionalUsd: 5_000 }).effectiveMaxNotionalUsd()).toBe(50_000);
    expect(new DonchianMtfPlugin().effectiveMaxNotionalUsd()).toBe(100_000);
  });

  it("createDonchianMtfPlugin factory works", () => {
    const p = createDonchianMtfPlugin();
    expect(p).toBeInstanceOf(DonchianMtfPlugin);
  });

  it("DEFAULT_DONCHIAN_MTF_PLUGIN_CONFIG has expected invariants", () => {
    expect(DEFAULT_DONCHIAN_MTF_PLUGIN_CONFIG.leverage).toBe(10);
    expect(DEFAULT_DONCHIAN_MTF_PLUGIN_CONFIG.baseNotionalUsd).toBe(10_000);
  });
  ...
  it("leverage=1 (baseline) accepted", () => {
    const p = new DonchianMtfPlugin({ leverage: 1 });
