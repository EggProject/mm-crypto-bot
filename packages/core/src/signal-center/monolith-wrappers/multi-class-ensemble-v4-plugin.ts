// packages/core/src/signal-center/monolith-wrappers/multi-class-ensemble-v4-plugin.ts
// — Phase 13 Track A
//
// ===========================================================================
// MultiClassEnsembleV4Plugin — wraps `MultiClassEnsembleV4` (multi-class-ensemble-v4)
// ===========================================================================
//
// The wrapper:
//   - holds a `MultiClassEnsembleV4` instance
...
/**
 * `MultiClassEnsembleV4PluginConfig` — configuration for the wrapper. Includes the
 * underlying strategy's knobs plus SCv1 envelope settings
 * (baseNotionalUsd, leverage).
 */
export interface MultiClassEnsembleV4PluginConfig {
  ...
  readonly strategy?: Partial<MultiClassEnsembleV4Config>;
  /** Leverage invariant config (Layer 2/3 reference). Default 1:10. */
  readonly leverageInvariant: LeverageInvariantConfig;
}

export const DEFAULT_MULTI_CLASS_ENSEMBLE_V4_PLUGIN_CONFIG: Omit<
  MultiClassEnsembleV4PluginConfig,
  "strategy"
> = {
...
/**
 * `MultiClassEnsembleV4PluginState` — per-plugin mutable state held across `onBar` calls.
 */
export interface MultiClassEnsembleV4PluginState {
...
export class MultiClassEnsembleV4Plugin implements StrategyPlugin {
  readonly metadata: StrategyPluginMetadata = {
    name: "multi-class-ensemble-v4-v1",
    version: "1.0.0",
    edgeClass: "mixed",
    capitalRequirement: 10_000,
    maxLeverage: ONE_TO_TEN_LEVERAGE, // 1:10 HARD GUARDRAIL
    description:
      "Phase 13 Track A wrapper around MultiClassEnsembleV4. " +
  ...
  readonly config: MultiClassEnsembleV4PluginConfig;
  readonly state: MultiClassEnsembleV4PluginState;
  /** Underlying monolith strategy. */
  private readonly underlying: MultiClassEnsembleV4;
  ...
  constructor(config: Partial<MultiClassEnsembleV4PluginConfig> = {}) {
    const merged: MultiClassEnsembleV4PluginConfig = {
      ...DEFAULT_MULTI_CLASS_ENSEMBLE_V4_PLUGIN_CONFIG,
      ...config,
    };
    // LAYER 1 — constructor check on metadata.maxLeverage.
    if (this.metadata.maxLeverage !== ONE_TO_TEN_LEVERAGE) {
      throw new Error(
        `[MultiClassEnsembleV4Plugin] 1:10 HARD GUARDRAIL VIOLATION: metadata.maxLeverage=${String(this.metadata.maxLeverage)} but the project-wide 1:10 mandate requires 10.`,
      );
    }
    // LAYER 1 — constructor check on leverage value.
    if (merged.leverage !== (1 as 1 | 10) && merged.leverage !== (10 as 1 | 10)) {
      throw new Error(
        `[MultiClassEnsembleV4Plugin] 1:10 HARD GUARDRAIL VIOLATION: leverage=${String(merged.leverage)} is NOT ALLOWED. Only 1 (baseline) or 10 (1:10 mandatory) are accepted.`,
      );
    }
    if (
      ...
      throw new Error(
        `[MultiClassEnsembleV4Plugin] baseNotionalUsd must be positive finite, got ${String(merged.baseNotionalUsd)}`,
      );
    }
    this.config = merged;
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    this.underlying = new MultiClassEnsembleV4({ ...DEFAULT_MULTI_CLASS_ENSEMBLE_V4_CONFIG_PARTIAL, ...merged.strategy } as unknown as MultiClassEnsembleV4Config);
  ...
  subscribe(bus: SignalBus): void {
    ...
    try {
      ...
      throw new Error(
        `[MultiClassEnsembleV4Plugin] LAYER 2 BREACH on subscribe: ${msg}`,
        { cause: e },
      );
    }
  }
  ...
  validateConfig(config: unknown): Result<void, ConfigError> {
    ...
    const c = config as Partial<MultiClassEnsembleV4PluginConfig>;
    if (
  ...
  private _mkState(): MultiClassEnsembleV4PluginState {
    return {
  ...
  private _emitSizing(strength: number, timestampMs: number): void {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[MultiClassEnsembleV4Plugin] LAYER 3 BREACH on sizing emit: ${msg}`,
        { cause: e },
      );
    }
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[MultiClassEnsembleV4Plugin] LAYER 3 BREACH post-clamp: ${msg}`,
        { cause: e },
      );
    }
...
/**
 * `createMultiClassEnsembleV4Plugin` — convenience factory.
 */
export function createMultiClassEnsembleV4Plugin(
  config?: Partial<MultiClassEnsembleV4PluginConfig>,
): MultiClassEnsembleV4Plugin {
  return new MultiClassEnsembleV4Plugin(config);
}
