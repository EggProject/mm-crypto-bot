// packages/core/src/signal-center/monolith-wrappers/donchian-trailing-plugin.ts
// — Phase 13 Track A
//
// ===========================================================================
// DonchianTrailingPlugin — wraps `DonchianTrailingStrategy` (donchian-trailing)
// ===========================================================================
//
// The wrapper:
//   - holds a `DonchianTrailingStrategy` instance
...
/**
 * `DonchianTrailingPluginConfig` — configuration for the wrapper. Includes the
 * underlying strategy's knobs plus SCv1 envelope settings
 * (baseNotionalUsd, leverage).
 */
export interface DonchianTrailingPluginConfig {
  ...
  readonly strategy?: Partial<DonchianTrailingConfig>;
  /** Leverage invariant config (Layer 2/3 reference). Default 1:10. */
  readonly leverageInvariant: LeverageInvariantConfig;
}

export const DEFAULT_DONCHIAN_TRAILING_PLUGIN_CONFIG: Omit<
  DonchianTrailingPluginConfig,
  "strategy"
> = {
...
/**
 * `DonchianTrailingPluginState` — per-plugin mutable state held across `onBar` calls.
 */
export interface DonchianTrailingPluginState {
...
export class DonchianTrailingPlugin implements StrategyPlugin {
  readonly metadata: StrategyPluginMetadata = {
    name: "donchian-trailing-v1",
    version: "1.0.0",
    edgeClass: "directional",
    capitalRequirement: 10_000,
    maxLeverage: ONE_TO_TEN_LEVERAGE, // 1:10 HARD GUARDRAIL
    description:
      "Phase 13 Track A wrapper around DonchianTrailingStrategy. " +
  ...
  readonly config: DonchianTrailingPluginConfig;
  readonly state: DonchianTrailingPluginState;
  /** Underlying monolith strategy. */
  private readonly underlying: DonchianTrailingStrategy;
  ...
  constructor(config: Partial<DonchianTrailingPluginConfig> = {}) {
    const merged: DonchianTrailingPluginConfig = {
      ...DEFAULT_DONCHIAN_TRAILING_PLUGIN_CONFIG,
      ...config,
    };
    // LAYER 1 — constructor check on metadata.maxLeverage.
    if (this.metadata.maxLeverage !== ONE_TO_TEN_LEVERAGE) {
      throw new Error(
        `[DonchianTrailingPlugin] 1:10 HARD GUARDRAIL VIOLATION: metadata.maxLeverage=${String(this.metadata.maxLeverage)} but the project-wide 1:10 mandate requires 10.`,
      );
    }
    // LAYER 1 — constructor check on leverage value.
    if (merged.leverage !== (1 as 1 | 10) && merged.leverage !== (10 as 1 | 10)) {
      throw new Error(
        `[DonchianTrailingPlugin] 1:10 HARD GUARDRAIL VIOLATION: leverage=${String(merged.leverage)} is NOT ALLOWED. Only 1 (baseline) or 10 (1:10 mandatory) are accepted.`,
      );
    }
    if (
      ...
      throw new Error(
        `[DonchianTrailingPlugin] baseNotionalUsd must be positive finite, got ${String(merged.baseNotionalUsd)}`,
      );
    }
    this.config = merged;
    this.underlying = new DonchianTrailingStrategy({ ...DEFAULT_DONCHIAN_TRAILING_CONFIG, ...merged.strategy });
  ...
  subscribe(bus: SignalBus): void {
    ...
    try {
      ...
      throw new Error(
        `[DonchianTrailingPlugin] LAYER 2 BREACH on subscribe: ${msg}`,
        { cause: e },
      );
    }
  }
  ...
  validateConfig(config: unknown): Result<void, ConfigError> {
    ...
    const c = config as Partial<DonchianTrailingPluginConfig>;
    if (
  ...
  private _mkState(): DonchianTrailingPluginState {
    return {
  ...
  private _emitSizing(strength: number, timestampMs: number): void {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[DonchianTrailingPlugin] LAYER 3 BREACH on sizing emit: ${msg}`,
        { cause: e },
      );
    }
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[DonchianTrailingPlugin] LAYER 3 BREACH post-clamp: ${msg}`,
        { cause: e },
      );
    }
...
/**
 * `createDonchianTrailingPlugin` — convenience factory.
 */
export function createDonchianTrailingPlugin(
  config?: Partial<DonchianTrailingPluginConfig>,
): DonchianTrailingPlugin {
  return new DonchianTrailingPlugin(config);
}
