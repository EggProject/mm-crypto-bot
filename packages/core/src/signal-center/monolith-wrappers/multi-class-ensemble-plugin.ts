// packages/core/src/signal-center/monolith-wrappers/multi-class-ensemble-plugin.ts
// — Phase 13 Track A
//
// ===========================================================================
// MultiClassEnsemblePlugin — wraps `MultiClassEnsemble` (multi-class-ensemble)
// ===========================================================================
//
// The wrapper:
//   - holds a `MultiClassEnsemble` instance
...
/**
 * `MultiClassEnsemblePluginConfig` — configuration for the wrapper. Includes the
 * underlying strategy's knobs plus SCv1 envelope settings
 * (baseNotionalUsd, leverage).
 */
export interface MultiClassEnsemblePluginConfig {
  ...
  readonly strategy?: Partial<MultiClassEnsembleConfig>;
  /** Leverage invariant config (Layer 2/3 reference). Default 1:10. */
  readonly leverageInvariant: LeverageInvariantConfig;
}

export const DEFAULT_MULTI_CLASS_ENSEMBLE_PLUGIN_CONFIG: Omit<
  MultiClassEnsemblePluginConfig,
  "strategy"
> = {
...
/**
 * `MultiClassEnsemblePluginState` — per-plugin mutable state held across `onBar` calls.
 */
export interface MultiClassEnsemblePluginState {
...
export class MultiClassEnsemblePlugin implements StrategyPlugin {
  readonly metadata: StrategyPluginMetadata = {
    name: "multi-class-ensemble-v1",
    version: "1.0.0",
    edgeClass: "mixed",
    capitalRequirement: 10_000,
    maxLeverage: ONE_TO_TEN_LEVERAGE, // 1:10 HARD GUARDRAIL
    description:
      "Phase 13 Track A wrapper around MultiClassEnsemble. " +
  ...
  readonly config: MultiClassEnsemblePluginConfig;
  readonly state: MultiClassEnsemblePluginState;
  /** Underlying monolith strategy. */
  private readonly underlying: MultiClassEnsemble;
  ...
  constructor(config: Partial<MultiClassEnsemblePluginConfig> = {}) {
    const merged: MultiClassEnsemblePluginConfig = {
      ...DEFAULT_MULTI_CLASS_ENSEMBLE_PLUGIN_CONFIG,
      ...config,
    };
    // LAYER 1 — constructor check on metadata.maxLeverage.
    if (this.metadata.maxLeverage !== ONE_TO_TEN_LEVERAGE) {
      throw new Error(
        `[MultiClassEnsemblePlugin] 1:10 HARD GUARDRAIL VIOLATION: metadata.maxLeverage=${String(this.metadata.maxLeverage)} but the project-wide 1:10 mandate requires 10.`,
      );
    }
    // LAYER 1 — constructor check on leverage value.
    if (merged.leverage !== (1 as 1 | 10) && merged.leverage !== (10 as 1 | 10)) {
      throw new Error(
        `[MultiClassEnsemblePlugin] 1:10 HARD GUARDRAIL VIOLATION: leverage=${String(merged.leverage)} is NOT ALLOWED. Only 1 (baseline) or 10 (1:10 mandatory) are accepted.`,
      );
    }
    if (
      ...
      throw new Error(
        `[MultiClassEnsemblePlugin] baseNotionalUsd must be positive finite, got ${String(merged.baseNotionalUsd)}`,
      );
    }
    this.config = merged;
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    this.underlying = new MultiClassEnsemble({ ...DEFAULT_MULTI_CLASS_ENSEMBLE_CONFIG_PARTIAL, ...merged.strategy } as unknown as MultiClassEnsembleConfig);
  ...
  subscribe(bus: SignalBus): void {
    ...
    try {
      ...
      throw new Error(
        `[MultiClassEnsemblePlugin] LAYER 2 BREACH on subscribe: ${msg}`,
        { cause: e },
      );
    }
  }
  ...
  validateConfig(config: unknown): Result<void, ConfigError> {
    ...
    const c = config as Partial<MultiClassEnsemblePluginConfig>;
    if (
  ...
  private _mkState(): MultiClassEnsemblePluginState {
    return {
  ...
  private _emitSizing(strength: number, timestampMs: number): void {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[MultiClassEnsemblePlugin] LAYER 3 BREACH on sizing emit: ${msg}`,
        { cause: e },
      );
    }
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[MultiClassEnsemblePlugin] LAYER 3 BREACH post-clamp: ${msg}`,
        { cause: e },
      );
    }
...
/**
 * `createMultiClassEnsemblePlugin` — convenience factory.
 */
export function createMultiClassEnsemblePlugin(
  config?: Partial<MultiClassEnsemblePluginConfig>,
): MultiClassEnsemblePlugin {
  return new MultiClassEnsemblePlugin(config);
}
