// packages/core/src/signal-center/monolith-wrappers/funding-carry-plugin.test.ts
// — Phase 13 Track A
//
// Test suite for FundingCarryPlugin — ≥5 tests covering:
...
import {
  FundingCarryPlugin,
  createFundingCarryPlugin,
  DEFAULT_FUNDING_CARRY_PLUGIN_CONFIG,
} from "./funding-carry-plugin.js";
import type { Bar } from "../types.js";

const mkBus = (): SignalBus => new SignalBus({ mode: "backtest" });

const wirePlugin = (p: FundingCarryPlugin): SignalBus => {
  const bus = mkBus();
  p.subscribe(bus);
  return bus;
};
...
describe("FundingCarryPlugin", () => {
  it("construction with default config succeeds", () => {
    const p = new FundingCarryPlugin();
  ...
  it("metadata declares maxLeverage=10 (1:10 HARD GUARDRAIL)", () => {
    ...
    expect(p.metadata.name).toBe("funding-carry-v1");
    expect(p.metadata.edgeClass).toBe("carry");
  });

  it("registry accepts the plugin", () => {
    const registry = new StrategyRegistry();
    registry.register(new FundingCarryPlugin());
  ...
  it("onBar emits a DirectionSignal (flat by default with minimal context)", () => {
    ...
    expect(sig.source).toBe("funding-carry-v1");
  });
  ...
  it("construction rejects leverage ∉ {1, 10}", () => {
    expect(() => new FundingCarryPlugin({ leverage: 5 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
    expect(() => new FundingCarryPlugin({ leverage: 0 as 1 | 10 })).toThrow(/1:10 HARD GUARDRAIL/);
  });

  it("construction rejects non-positive baseNotionalUsd", () => {
    expect(() => new FundingCarryPlugin({ baseNotionalUsd: 0 })).toThrow(/baseNotionalUsd/);
    expect(() => new FundingCarryPlugin({ baseNotionalUsd: -1 })).toThrow(/baseNotionalUsd/);
  });
  ...
  it("effectiveMaxNotionalUsd is baseNotional × 10", () => {
    expect(new FundingCarryPlugin({ baseNotionalUsd: 5_000 }).effectiveMaxNotionalUsd()).toBe(50_000);
    expect(new FundingCarryPlugin().effectiveMaxNotionalUsd()).toBe(100_000);
  });

  it("createFundingCarryPlugin factory works", () => {
    const p = createFundingCarryPlugin();
    expect(p).toBeInstanceOf(FundingCarryPlugin);
  });

  it("DEFAULT_FUNDING_CARRY_PLUGIN_CONFIG has expected invariants", () => {
    expect(DEFAULT_FUNDING_CARRY_PLUGIN_CONFIG.leverage).toBe(10);
    expect(DEFAULT_FUNDING_CARRY_PLUGIN_CONFIG.baseNotionalUsd).toBe(10_000);
  });
  ...
  it("leverage=1 (baseline) accepted", () => {
    const p = new FundingCarryPlugin({ leverage: 1 });
