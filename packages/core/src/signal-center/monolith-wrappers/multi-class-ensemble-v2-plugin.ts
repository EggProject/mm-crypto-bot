// packages/core/src/signal-center/monolith-wrappers/multi-class-ensemble-v2-plugin.ts
// — Phase 13 Track A
//
// ===========================================================================
// MultiClassEnsembleV2Plugin — wraps `MultiClassEnsembleV2` (multi-class-ensemble-v2)
// ===========================================================================
//
// The wrapper:
//   - holds a `MultiClassEnsembleV2` instance
...
/**
 * `MultiClassEnsembleV2PluginConfig` — configuration for the wrapper. Includes the
 * underlying strategy's knobs plus SCv1 envelope settings
 * (baseNotionalUsd, leverage).
 */
export interface MultiClassEnsembleV2PluginConfig {
  ...
  readonly strategy?: Partial<MultiClassEnsembleV2Config>;
  /** Leverage invariant config (Layer 2/3 reference). Default 1:10. */
  readonly leverageInvariant: LeverageInvariantConfig;
}

export const DEFAULT_MULTI_CLASS_ENSEMBLE_V2_PLUGIN_CONFIG: Omit<
  MultiClassEnsembleV2PluginConfig,
  "strategy"
> = {
...
/**
 * `MultiClassEnsembleV2PluginState` — per-plugin mutable state held across `onBar` calls.
 */
export interface MultiClassEnsembleV2PluginState {
...
export class MultiClassEnsembleV2Plugin implements StrategyPlugin {
  readonly metadata: StrategyPluginMetadata = {
    name: "multi-class-ensemble-v2-v1",
    version: "1.0.0",
    edgeClass: "mixed",
    capitalRequirement: 10_000,
    maxLeverage: ONE_TO_TEN_LEVERAGE, // 1:10 HARD GUARDRAIL
    description:
      "Phase 13 Track A wrapper around MultiClassEnsembleV2. " +
  ...
  readonly config: MultiClassEnsembleV2PluginConfig;
  readonly state: MultiClassEnsembleV2PluginState;
  /** Underlying monolith strategy. */
  private readonly underlying: MultiClassEnsembleV2;
  ...
  constructor(config: Partial<MultiClassEnsembleV2PluginConfig> = {}) {
    const merged: MultiClassEnsembleV2PluginConfig = {
      ...DEFAULT_MULTI_CLASS_ENSEMBLE_V2_PLUGIN_CONFIG,
      ...config,
    };
    // LAYER 1 — constructor check on metadata.maxLeverage.
    if (this.metadata.maxLeverage !== ONE_TO_TEN_LEVERAGE) {
      throw new Error(
        `[MultiClassEnsembleV2Plugin] 1:10 HARD GUARDRAIL VIOLATION: metadata.maxLeverage=${String(this.metadata.maxLeverage)} but the project-wide 1:10 mandate requires 10.`,
      );
    }
    // LAYER 1 — constructor check on leverage value.
    if (merged.leverage !== (1 as 1 | 10) && merged.leverage !== (10 as 1 | 10)) {
      throw new Error(
        `[MultiClassEnsembleV2Plugin] 1:10 HARD GUARDRAIL VIOLATION: leverage=${String(merged.leverage)} is NOT ALLOWED. Only 1 (baseline) or 10 (1:10 mandatory) are accepted.`,
      );
    }
    if (
      ...
      throw new Error(
        `[MultiClassEnsembleV2Plugin] baseNotionalUsd must be positive finite, got ${String(merged.baseNotionalUsd)}`,
      );
    }
    this.config = merged;
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    this.underlying = new MultiClassEnsembleV2({ ...DEFAULT_MULTI_CLASS_ENSEMBLE_V2_CONFIG_PARTIAL, ...merged.strategy } as unknown as MultiClassEnsembleV2Config);
  ...
  subscribe(bus: SignalBus): void {
    ...
    try {
      ...
      throw new Error(
        `[MultiClassEnsembleV2Plugin] LAYER 2 BREACH on subscribe: ${msg}`,
        { cause: e },
      );
    }
  }
  ...
  validateConfig(config: unknown): Result<void, ConfigError> {
    ...
    const c = config as Partial<MultiClassEnsembleV2PluginConfig>;
    if (
  ...
  private _mkState(): MultiClassEnsembleV2PluginState {
    return {
  ...
  private _emitSizing(strength: number, timestampMs: number): void {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[MultiClassEnsembleV2Plugin] LAYER 3 BREACH on sizing emit: ${msg}`,
        { cause: e },
      );
    }
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[MultiClassEnsembleV2Plugin] LAYER 3 BREACH post-clamp: ${msg}`,
        { cause: e },
      );
    }
...
/**
 * `createMultiClassEnsembleV2Plugin` — convenience factory.
 */
export function createMultiClassEnsembleV2Plugin(
  config?: Partial<MultiClassEnsembleV2PluginConfig>,
): MultiClassEnsembleV2Plugin {
  return new MultiClassEnsembleV2Plugin(config);
}
