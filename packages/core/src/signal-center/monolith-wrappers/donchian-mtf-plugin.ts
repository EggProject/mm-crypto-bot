// packages/core/src/signal-center/monolith-wrappers/donchian-mtf-plugin.ts
// — Phase 13 Track A
//
// ===========================================================================
// DonchianMtfPlugin — wraps `DonchianMtfStrategy` (donchian-mtf)
// ===========================================================================
//
// The wrapper:
//   - holds a `DonchianMtfStrategy` instance
...
/**
 * `DonchianMtfPluginConfig` — configuration for the wrapper. Includes the
 * underlying strategy's knobs plus SCv1 envelope settings
 * (baseNotionalUsd, leverage).
 */
export interface DonchianMtfPluginConfig {
  ...
  readonly strategy?: Partial<DonchianMtfConfig>;
  /** Leverage invariant config (Layer 2/3 reference). Default 1:10. */
  readonly leverageInvariant: LeverageInvariantConfig;
}

export const DEFAULT_DONCHIAN_MTF_PLUGIN_CONFIG: Omit<
  DonchianMtfPluginConfig,
  "strategy"
> = {
...
/**
 * `DonchianMtfPluginState` — per-plugin mutable state held across `onBar` calls.
 */
export interface DonchianMtfPluginState {
...
export class DonchianMtfPlugin implements StrategyPlugin {
  ...
  readonly config: DonchianMtfPluginConfig;
  readonly state: DonchianMtfPluginState;
  /** Underlying monolith strategy. */
  private readonly underlying: DonchianMtfStrategy;
  ...
  constructor(config: Partial<DonchianMtfPluginConfig> = {}) {
    const merged: DonchianMtfPluginConfig = {
      ...DEFAULT_DONCHIAN_MTF_PLUGIN_CONFIG,
      ...config,
    };
    // LAYER 1 — constructor check on metadata.maxLeverage.
    if (this.metadata.maxLeverage !== ONE_TO_TEN_LEVERAGE) {
      throw new Error(
        `[DonchianMtfPlugin] 1:10 HARD GUARDRAIL VIOLATION: metadata.maxLeverage=${String(this.metadata.maxLeverage)} but the project-wide 1:10 mandate requires 10.`,
      );
    }
    // LAYER 1 — constructor check on leverage value.
    if (merged.leverage !== (1 as 1 | 10) && merged.leverage !== (10 as 1 | 10)) {
      throw new Error(
        `[DonchianMtfPlugin] 1:10 HARD GUARDRAIL VIOLATION: leverage=${String(merged.leverage)} is NOT ALLOWED. Only 1 (baseline) or 10 (1:10 mandatory) are accepted.`,
      );
    }
    if (
      ...
      throw new Error(
        `[DonchianMtfPlugin] baseNotionalUsd must be positive finite, got ${String(merged.baseNotionalUsd)}`,
      );
    }
    this.config = merged;
    this.underlying = new DonchianMtfStrategy({ ...DEFAULT_DONCHIAN_MTF_CONFIG, ...merged.strategy });
  ...
  subscribe(bus: SignalBus): void {
    ...
    try {
      ...
      throw new Error(
        `[DonchianMtfPlugin] LAYER 2 BREACH on subscribe: ${msg}`,
        { cause: e },
      );
    }
  }
  ...
  validateConfig(config: unknown): Result<void, ConfigError> {
    ...
    const c = config as Partial<DonchianMtfPluginConfig>;
    if (
  ...
  private _mkState(): DonchianMtfPluginState {
    return {
  ...
  private _emitSizing(strength: number, timestampMs: number): void {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[DonchianMtfPlugin] LAYER 3 BREACH on sizing emit: ${msg}`,
        { cause: e },
      );
    }
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[DonchianMtfPlugin] LAYER 3 BREACH post-clamp: ${msg}`,
        { cause: e },
      );
    }
...
/**
 * `createDonchianMtfPlugin` — convenience factory.
 */
export function createDonchianMtfPlugin(
  config?: Partial<DonchianMtfPluginConfig>,
): DonchianMtfPlugin {
  return new DonchianMtfPlugin(config);
}
