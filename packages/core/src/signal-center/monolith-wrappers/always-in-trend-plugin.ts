// packages/core/src/signal-center/monolith-wrappers/always-in-trend-plugin.ts
// — Phase 13 Track A
//
// ===========================================================================
// AlwaysInTrendPlugin — wraps `AlwaysInTrendStrategy` (Phase 5)
// ===========================================================================
//
// The wrapper:
//   - holds an `AlwaysInTrendStrategy` instance
...
//
// The wrapper's `onBar` produces a "flat" DirectionSignal whenever the
// underlying returns null (or when the StrategyContext has insufficient
// indicator state — the engine does not have MTF aggregation logic;
// callers that need full MTF Donchian fidelity should use
// DirectionalMTFPlugin instead, which implements full MTF rollup).
//
...
import {
  AlwaysInTrendStrategy,
  type AlwaysInTrendConfig,
  DEFAULT_ALWAYSIN_CONFIG,
} from "../../strategy/always-in-trend.js";
import {
  ONE_TO_TEN_LEVERAGE,
...
/**
 * `AlwaysInTrendPluginConfig` — configuration for the wrapper. Includes
 * the underlying strategy's knobs plus SCv1 envelope settings
 * (baseNotionalUsd, leverage).
 */
export interface AlwaysInTrendPluginConfig {
  ...
  /** Pass-through to `AlwaysInTrendStrategy`. */
  readonly strategy?: Partial<AlwaysInTrendConfig>;
  /** Leverage invariant config (Layer 2/3 reference). Default 1:10. */
  readonly leverageInvariant: LeverageInvariantConfig;
}

export const DEFAULT_ALWAYS_IN_TREND_PLUGIN_CONFIG: Omit<
  AlwaysInTrendPluginConfig,
  "strategy"
> = {
...
/**
 * `AlwaysInTrendPluginState` — per-plugin mutable state held across
 * `onBar` calls. Includes the underlying strategy + emission bookkeeping.
 */
export interface AlwaysInTrendPluginState {
...
export class AlwaysInTrendPlugin implements StrategyPlugin {
  readonly metadata: StrategyPluginMetadata = {
    name: "always-in-trend-v1",
    version: "1.0.0",
    edgeClass: "directional",
    capitalRequirement: 10_000,
    maxLeverage: ONE_TO_TEN_LEVERAGE, // 1:10 HARD GUARDRAIL
    description:
      "Phase 13 Track A wrapper around Phase 5 AlwaysInTrendStrategy. " +
  ...
  readonly config: AlwaysInTrendPluginConfig;
  readonly state: AlwaysInTrendPluginState;
  /** Underlying monolith strategy. */
  private readonly underlying: AlwaysInTrendStrategy;
  ...
  constructor(config: Partial<AlwaysInTrendPluginConfig> = {}) {
    const merged: AlwaysInTrendPluginConfig = {
    ...
    if (this.metadata.maxLeverage !== ONE_TO_TEN_LEVERAGE) {
      throw new Error(
        `[AlwaysInTrendPlugin] 1:10 HARD GUARDRAIL VIOLATION: metadata.maxLeverage=${String(this.metadata.maxLeverage)} but the project-wide 1:10 mandate requires 10.`,
      );
    }
    // LAYER 1 — constructor check on leverage value.
    if (merged.leverage !== (1 as 1 | 10) && merged.leverage !== (10 as 1 | 10)) {
      throw new Error(
        `[AlwaysInTrendPlugin] 1:10 HARD GUARDRAIL VIOLATION: leverage=${String(merged.leverage)} is NOT ALLOWED. Only 1 (baseline) or 10 (1:10 mandatory) are accepted.`,
      );
    }
    if (
      ...
      throw new Error(
        `[AlwaysInTrendPlugin] baseNotionalUsd must be positive finite, got ${String(merged.baseNotionalUsd)}`,
      );
    }
    this.config = merged;
    this.underlying = new AlwaysInTrendStrategy({
      ...DEFAULT_ALWAYSIN_CONFIG,
      ...merged.strategy,
    });
  ...
  subscribe(bus: SignalBus): void {
    ...
    try {
      ...
      throw new Error(
        `[AlwaysInTrendPlugin] LAYER 2 BREACH on subscribe: ${msg}`,
        { cause: e },
      );
    }
  }
  ...
  onBar(bar: Bar, _state: PluginState): void {
    this.barCount += 1;
    this.candleIndex += 1;
    // Build a minimal StrategyContext from the bar. The wrapper does
    // NOT aggregate multi-timeframe state — the underlying strategy
    // returns null when MTF indicators are missing (which is the
    // expected behavior for a single-bar context).
    const ctx = this._buildContext(bar);
    let signal: StrategySignal | null = null;
    try {
      signal = this.underlying.onCandle(ctx);
    } catch (e: unknown) {
      // Defensive — strategy threw on benign input; swallow + log.
      void e;
    }
  ...
  validateConfig(config: unknown): Result<void, ConfigError> {
    ...
    const c = config as Partial<AlwaysInTrendPluginConfig>;
    if (
   ...
   * sizing emit path with a synthetic strength value. Used by tests to
   * exercise the 1:10 clamp + assertion branches without requiring full
   * MTF state to drive the underlying strategy.
   */
  ...
  private _mkState(): AlwaysInTrendPluginState {
    return {
  ...
  /**
   * `_buildContext` — build a minimal `StrategyContext` for the bar.
   * Note: mtfState is populated with `undefined` indicator fields; the
   * underlying strategy returns null when these are absent, which is
   * the wrapper's "flat" signal.
   */
  ...
  /**
   * `_emitFromSignal` — translate a `StrategySignal` into typed Signal
   * events. `side: "buy"` → DirectionSignal "long" + SizingSignal.
   * `side: "sell"` → DirectionSignal "short". `null` → DirectionSignal
   * "flat".
   */
  ...
  /**
   * `_emitSizing` — compose + emit a SizingSignal with Layer 3 1:10 cap.
   */
  private _emitSizing(strength: number, timestampMs: number): void {
    ...
    let notional =
      ...
      kellyFraction *
      volMultiplier;
    // Layer 3 — assert computed notional satisfies the invariant.
    try {
      assertLeverageInvariant(
        notional,
        this.config.baseNotionalUsd,
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[AlwaysInTrendPlugin] LAYER 3 BREACH on sizing emit: ${msg}`,
        { cause: e },
      );
    }
    // Hard clamp to the 1:10 ceiling.
    const maxNotional = this.effectiveMaxNotionalUsd();
    if (notional > maxNotional) {
      notional = maxNotional;
      this.state.leverageClampCount += 1;
    }
    // Re-assert after clamp.
    try {
      assertLeverageInvariant(
        notional,
        this.config.baseNotionalUsd,
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[AlwaysInTrendPlugin] LAYER 3 BREACH post-clamp: ${msg}`,
        { cause: e },
      );
    }
...
/**
 * `createAlwaysInTrendPlugin` — convenience factory.
 */
export function createAlwaysInTrendPlugin(
  config?: Partial<AlwaysInTrendPluginConfig>,
): AlwaysInTrendPlugin {
  return new AlwaysInTrendPlugin(config);
}