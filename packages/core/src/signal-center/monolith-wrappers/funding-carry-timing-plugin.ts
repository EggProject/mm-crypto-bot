// packages/core/src/signal-center/monolith-wrappers/funding-carry-timing-plugin.ts
// — Phase 13 Track A
//
// ===========================================================================
// FundingCarryTimingPlugin — wraps `FundingCarryTimingStrategy` (funding-carry-timing)
// ===========================================================================
//
// The wrapper:
//   - holds a `FundingCarryTimingStrategy` instance
...
/**
 * `FundingCarryTimingPluginConfig` — configuration for the wrapper. Includes the
 * underlying strategy's knobs plus SCv1 envelope settings
 * (baseNotionalUsd, leverage).
 */
export interface FundingCarryTimingPluginConfig {
  ...
  readonly strategy?: Partial<FundingCarryTimingConfig>;
  /** Leverage invariant config (Layer 2/3 reference). Default 1:10. */
  readonly leverageInvariant: LeverageInvariantConfig;
}

export const DEFAULT_FUNDING_CARRY_TIMING_PLUGIN_CONFIG: Omit<
  FundingCarryTimingPluginConfig,
  "strategy"
> = {
...
/**
 * `FundingCarryTimingPluginState` — per-plugin mutable state held across `onBar` calls.
 */
export interface FundingCarryTimingPluginState {
...
export class FundingCarryTimingPlugin implements StrategyPlugin {
  readonly metadata: StrategyPluginMetadata = {
    name: "funding-carry-timing-v1",
    version: "1.0.0",
    edgeClass: "carry",
    capitalRequirement: 10_000,
    maxLeverage: ONE_TO_TEN_LEVERAGE, // 1:10 HARD GUARDRAIL
    description:
      "Phase 13 Track A wrapper around FundingCarryTimingStrategy. " +
  ...
  readonly config: FundingCarryTimingPluginConfig;
  readonly state: FundingCarryTimingPluginState;
  /** Underlying monolith strategy. */
  private readonly underlying: FundingCarryTimingStrategy;
  ...
  constructor(config: Partial<FundingCarryTimingPluginConfig> = {}) {
    const merged: FundingCarryTimingPluginConfig = {
      ...DEFAULT_FUNDING_CARRY_TIMING_PLUGIN_CONFIG,
      ...config,
    };
    // LAYER 1 — constructor check on metadata.maxLeverage.
    if (this.metadata.maxLeverage !== ONE_TO_TEN_LEVERAGE) {
      throw new Error(
        `[FundingCarryTimingPlugin] 1:10 HARD GUARDRAIL VIOLATION: metadata.maxLeverage=${String(this.metadata.maxLeverage)} but the project-wide 1:10 mandate requires 10.`,
      );
    }
    // LAYER 1 — constructor check on leverage value.
    if (merged.leverage !== (1 as 1 | 10) && merged.leverage !== (10 as 1 | 10)) {
      throw new Error(
        `[FundingCarryTimingPlugin] 1:10 HARD GUARDRAIL VIOLATION: leverage=${String(merged.leverage)} is NOT ALLOWED. Only 1 (baseline) or 10 (1:10 mandatory) are accepted.`,
      );
    }
    if (
      ...
      throw new Error(
        `[FundingCarryTimingPlugin] baseNotionalUsd must be positive finite, got ${String(merged.baseNotionalUsd)}`,
      );
    }
    this.config = merged;
    this.underlying = new FundingCarryTimingStrategy({ ...DEFAULT_FUNDING_CARRY_TIMING_CONFIG, ...merged.strategy });
  ...
  subscribe(bus: SignalBus): void {
    ...
    try {
      ...
      throw new Error(
        `[FundingCarryTimingPlugin] LAYER 2 BREACH on subscribe: ${msg}`,
        { cause: e },
      );
    }
  }
  ...
  validateConfig(config: unknown): Result<void, ConfigError> {
    ...
    const c = config as Partial<FundingCarryTimingPluginConfig>;
    if (
  ...
  private _mkState(): FundingCarryTimingPluginState {
    return {
  ...
  private _emitSizing(strength: number, timestampMs: number): void {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[FundingCarryTimingPlugin] LAYER 3 BREACH on sizing emit: ${msg}`,
        { cause: e },
      );
    }
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `[FundingCarryTimingPlugin] LAYER 3 BREACH post-clamp: ${msg}`,
        { cause: e },
      );
    }
...
/**
 * `createFundingCarryTimingPlugin` — convenience factory.
 */
export function createFundingCarryTimingPlugin(
  config?: Partial<FundingCarryTimingPluginConfig>,
): FundingCarryTimingPlugin {
  return new FundingCarryTimingPlugin(config);
}
