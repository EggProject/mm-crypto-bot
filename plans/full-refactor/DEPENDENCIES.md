...
### C4c internal logging workspace evidence

The C4c implementation adds private internal workspace package
`@mm-crypto-bot/logging` with no runtime external dependency. `apps/bot` uses
only the internal `workspace:*` link; the lock changes only add the workspace
resolution and existing exact-pinned Vitest/TypeScript development tooling.
The lifecycle-disabled frozen install completed with no changes. `bun pm
untrusted` reports Lefthook only; no new trusted dependency or hook activation
was introduced. `bun audit` returned `ConnectionRefused`, so a current
vulnerability result is **NOT EVIDENCED**. Exact lock/hash evidence is in
...
TypeScript `6.0.3`, Turbo `2.10.2`, ESLint `10.6.0`, and exact CCXT `4.5.75`.
The published ESM artifact self-reports `4.5.74`, which is a HIGH upstream
release-integrity blocker: the dependency remains exact per the approved
requirement, but no live dependency-readiness claim is permitted until an
upstream correction or official clarification is revalidated.
...
| Deferred boundaries          | At this 2026-08-17 record, CCXT was `4.5.64` and the audited candidate was `4.5.70`. This historical row is superseded for CCXT by exact `4.5.75`; its ESM artifact mismatch is currently blocked. Zod is `3.25.76`; possible `4.4.3` migration is breaking. `fraction.js` `5.3.4` belongs only to the numeric-package phase. | [CCXT](https://github.com/ccxt/ccxt), [Zod](https://zod.dev/), and [fraction.js](https://github.com/rawify/Fraction.js). These are deliberately not part of a generic sweep.                                                                                                                                                                  |
...
trusting Lefthook: at that recorded time, `trustedDependencies` contained only
`ccxt`. This historical state is superseded by the current empty trust list.
Automated
...
| CCXT `4.5.64 -> 4.5.70`               | **Historical proposal, superseded.** The active exact pin is `4.5.75`, with a recorded ESM release-integrity blocker. | Completed Phase 3 D-04 `PublicHistoricalClient` contract and completed Phase 4 Bybit EU safety evidence. | Fail-closed/no-network wrapper tests, provider authority/AST negatives, exact-10x/eligibility/borrow safety tests, frozen-install/audit/license evidence, and upstream ESM artifact correction or official clarification. |
...
| Evidence                         | Result                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Manifest policy                  | Historical record: all root and current workspace external declarations were exact; internal dependencies remained `workspace:*`. Applied approved targets: `@tsconfig/bases` `1.0.27`, `@types/node` `24.13.3`, Turbo `2.10.10`, `protobufjs` `8.7.2`, `smol-toml` `1.8.0`, `write-file-atomic` `8.0.0`, and `picocolors` `1.1.1`. Its CCXT `4.5.64` statement is superseded by exact `4.5.75` with the separately recorded ESM-integrity blocker; Zod remained exact `3.25.76`; fraction.js was not introduced. |
...
| Lifecycle inspection             | Historical record: `ccxt@4.5.64` and `lefthook@2.1.10` were inspected locally. Both declare `postinstall`; Lefthook declares platform optional binaries. Its trust state is superseded by the current empty `trustedDependencies` list; both packages are intentionally untrusted.                                                                                                                                                                                                                                |
...
| Trust and script metadata                                                                                                                 | Historical record: root `trustedDependencies` was exactly `ccxt`; Lefthook was absent. This is superseded by the current empty trust list. `bun pm untrusted` exited 0 and printed `Found 0 untrusted dependencies with scripts.` This is an observed Bun result, not proof that Lefthook is trusted, lifecycle-safe, audited, or activated. |
...
| Lock/manifest trust identity                                            | Historical record: `package.json` and regenerated `bun.lock` serialized only `ccxt` under `trustedDependencies`; no Lefthook entry. This is superseded by the current empty trust list. Lock SHA-256: `442b050e7922a89ecba1ca262d5a8c869de771af068ffd19e94d0a10a8407b40`. |
...
| Existing CCXT                                                                                                  | Exact-pinned `4.5.75` is installed. Its ESM artifact mismatch is blocked pending upstream correction or official clarification; no downgrade or alternate pin is authorized. | Sole live adapter plus D-04 public research adapter.                | P3 `PublicHistoricalClient` plus P4 Bybit EU fail-closed/no-network/safety evidence and a fresh release-integrity revalidation.                                                                                                                                                                                   |
...
postinstall; this historical record had `trustedDependencies` exactly `ccxt`.
The current list is empty. The isolated
