...
| D-10 | APPROVED: Bybit EU connection contract                            | Use only `ccxt.pro.bybiteu` with fixed official Bybit EU origins. Reject testnet/sandbox and manual REST or WebSocket origin overrides. Use WebSocket-first where a supported CCXT Pro/Bybit EU capability and fail-closed contract permit it; retain REST only when no appropriate WebSocket API exists or authoritative preflight/reconciliation requires it. Retain built-in paper/emulated execution, optionally driven by real public Bybit EU market data, but never submit an exchange order from that mode. | Live connection, external endpoint, execution-transport, and paper-mode semantics.                                                                                                                        | P4 config/factory/feed migration, later WebSocket order/action migration, capability and fail-closed tests, reconciliation tests, and independent review. This record does not establish implementation PASS or live readiness. |
...
**Status:** APPROVED on 2026-08-31 (Europe/Budapest), approver: user. Immutable session reference: `session=mm-crypto-bot-fixes`. The exact event wall-clock is not exposed by the interface.
