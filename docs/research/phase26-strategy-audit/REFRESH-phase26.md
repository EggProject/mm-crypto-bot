...
1. Wiped `backtest-results/` of all 272 old JSON logs (baseline-_, mr-baseline-_, phase15-* through
   phase25-_, audit-_, sensitivity-_, wf-_, portfolio-orchestrator/, oos-_, etc.) using `mavis-trash`
   (recoverable). Kept only research reports (REPORT-_.md), DROP-RETAIN-decisions.json,
