...
- `CascadeFadeStrategy.onCandle(_context: unknown): undefined` — explicit no-op at `packages/core/src/strategy/cascade-fade-paper.ts:203`. The cascade detector cannot influence the Phase 19 #1 baseline engine loop.
...
See [the branch-state and caveats appendix](REPORT-phase25-2-appendix.md).
