...
**Verdict — Track E alpha for mm-crypto-bot: NEGATIVE for direct basis trading; MARGINAL for stablecoin-peg micro-arb; POSITIVE as a _structural read_ on EU retail crypto flow migration.**
...
3. The structural premise of "EU retail flow forces bybit.eu to trade at a premium" is contradicted by the fact that **EU retail has migrated away from perps entirely** (because bybit.eu doesn't offer them), so the demand-side pressure on EU spot is _lower_, not higher, than on global spot.
