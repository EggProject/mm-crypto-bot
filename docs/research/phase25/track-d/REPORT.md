...
cap=0.18), cascade-fade is a _satellite_ book (~+0.5-1.5%/mo realistic), not a
replacement core.
...
_derivative_ of the perpetual market rather than its anchor." [26]
...
**Implication for our pipeline:** Binance perp is the _first_ venue where cascade
...
2. **Cross-exchange aggregated view (~1-5s latency):**
   ...
   - CoinGlass `/api/futures/liquidation/heatmap/model2` is the _uniquely
     valuable_ signal — a model estimating where liquidation clusters are
...
3. **Bybit.eu SPOT execution channel (separate):**
   ...
   - During cascade, RPI (Retail Price Improvement) orders provide 50%+ of
     depth at 5-10bps from mid and 30% within 5bps — this is the _only_
...
bybit.eu SPOT fill.** This is _tight but tradeable_ — by minute 4-5 the
...
**Reconciliation:** curupira's edge is _cascade-specific_ (only fades
during high-vol events with confirmed OI drop); anomiq's test is
_unconditional_ across all moments. Both can be true: cascade-fade has a
...
This is _consistent_ with Phase 24 portfolio sizing (cap=0.18 = $90k on
...
- Bybit SPOT has ~7% global share (post-hack recovery), RPI orders provide
  consistent depth at 5-10bps — _better relative liquidity_ than mid-tier
...
_cascade confirmation_ (OI drop + liquidation spike + ELR drop), the signal
is noise.
...
- **Korean retail lending** (빗썸 렌딩플러스 / 업비트 코인빌리기) has produced
  **2,1301 강제청산** on 빗썸 alone by Sept 2025 (12.6% of users in July alone
  ...
  structurally different from perp-DEX cascades — it's spot-margin _retail_
...
- **Bybit.eu post-Feb-2025 hack** recovery is at 7% share, "with metrics
  ...
  Scholes [8]. The hack is _resolved_ but the relative-share lower base
...
The biggest empirical worry: on 2025-10-10, top-of-book depth on _key venues_
...
top-decile cascade may _fail to fill_ (price moves through the limit before
...
during _isolated_ cascades; a cascade-as-regime-change breaks this
assumption.
...
survive most plausible exchange failure scenarios — but a _replay_ of
...
Per Phase 14B user mandate ("DD 15% is fine, size to 15% DD"), this is _not_
the place to bet the portfolio. Phase 24 #1's +39.37%/mo at cap=0.18 / <8% DD
is the core. Cascade-fade is an _event-driven satellite_ sized at ~$500k-$1M
...
_meaningful but not transformational_; it's a margin-improvement, not
...
_End of REPORT.md. See `sources.md` for full bibliography (≥10 rows, ≥2 ko)._
