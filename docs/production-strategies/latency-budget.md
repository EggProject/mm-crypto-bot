...
The bot's strategy decisions are _latency-sensitive_ (the carry
...
- **Hop 5** is the exchange-internal matching latency. Bybit's
  ...
  across both profiles because it's _server-side_.
...
> **Sources:**
>
> 1. _Exchange Co-Location in the Cloud Era: AWS Local Zones_ —
...
> **Sources:**
>
> 1. _Latency statistics | Business | IIJ_ — iij.ad.jp/en/svcsol/
...
>    This is the floor for the _Japan domestic_ leg of the
>    round-trip.
> 2. _Internet Speed in Tokyo (Japan)_ — speedgeo.net/statistics/
>    japan/tokyo. Reports an **average residential latency of
>    37 ms** in Tokyo (measured to Speedtest.net's nearest PoP).
>    This is the _home broadband → Japan edge_ floor for a
>    residential Japanese user; a non-Japanese home user
>    (e.g. Budapest) adds another 100-200 ms on top.
> 3. _What is a Normal Ping Value for Japan Servers?_ —
>    simcentric.com/japan-dedicated-server/... — confirms the
>    _Japan Domestic = 5-20 ms optimal / 20-40 ms acceptable_
...
remains _correct_ (no logic errors), but the alpha on time-
...
> — the funding-rate window is 8 hours, but the _useful edge_
...
> **Sources for §5.4:**
>
> 1. _Exchange Co-Location in the Cloud Era_ (nikpadala.com)
>    — confirms Equinix ECX routing inside TY11; < 1 ms RTT
>    is the design target for cross-connected matching.
> 2. _Bybit Server Location_ (arbitron.app) — Bybit API
...
1. _Bybit Server Location — AWS Region & Latency (2026)_ —
...
2. _Exchange Co-Location in the Cloud Era: AWS Local Zones_ —
...
3. _Latency statistics | Business | IIJ_ —
...
4. _Internet Speed in Tokyo (Japan)_ —
...
5. _What is a Normal Ping Value for Japan Servers?_ —
...
6. _Crypto Exchange API Latency Benchmarked (2026) /
   Binance vs OKX vs Bybit_ —
...
7. _CCXT issue #11614: What is expected latency for
   placing an order_ — https://github.com/ccxt/ccxt/
...
8. _How to Improve Execution in Crypto Markets_ —
...
9. _DeFi HFT Infrastructure: AWS Tokyo_ —
...
10. _Bybit API Latency: Measure, Optimize, and Stabilize_ —
