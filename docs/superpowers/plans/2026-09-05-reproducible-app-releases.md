...
Tasks 5–7 continue in [`2026-09-05-reproducible-app-releases-tasks-4-6.md`](2026-09-05-reproducible-app-releases-tasks-4-6.md).
...
- A release-set candidate carries only its direct-child temporary directory, fixed basename, and archive path. `releaseSetCandidatePrefix = "release-set-candidate-" as const` is exported once from `release-set-contract.ts`; assembler `mkdtemp`, publisher validation, and every test import it. `publishReleaseSet` receives the trusted private root separately from its caller, so a forged candidate cannot choose that anchor. Before any port call, publisher-owned pure validation requires that caller-supplied root relationship, the exact constant prefix with a nonempty suffix, and the exact archive join; a wrong prefix or empty suffix makes no port call. The current restricted candidate filesystem port alone may later call `removePrivateDirectory(candidate.directory)`; it never operates on the public root or destination. The reproducibility orchestration explicitly supplies `releaseDependencies.temporaryRoot` as that trusted root.
- Release-set manifests always order `applications` as `["bot", "config-search"]`. `createReleaseSetManifest` and `encodeReleaseSetZip` accept either input permutation only when it has exactly one input for each app, canonicalize to that order, and therefore yield byte-identical results. The independent verifier rejects reversed or otherwise noncanonical application arrays.
- `encodeReleaseSetZip` receives no separate `sourceDateEpoch`: it derives every outer local and central ZIP-header normalized timestamp solely from the canonical set manifest's shared identity value. The independent verifier compares every outer local and central header timestamp to that normalized manifest value. Codec tests prove this single source, and mutated local or central timestamp mismatch tests fail closed.
- `assertReproducibleReleaseSet` immediately retains the ZIP, sidecar, and manifest bytes from each app's first independently read and verified candidate. For each canonical app, it then creates/verifies the second candidate, byte-compares it with that retained first snapshot, and smokes both; the retained snapshot is not eligible for outer assembly until those later gates pass. Only after both apps pass does it create exactly one outer candidate from those retained first snapshots; it never creates a third per-app candidate after the gate. Tests prove the outer entries are exactly those verified first-candidate bytes, not a later or raw candidate.
...
- The ten Task 7 owned changed/new runtime sources have 100% statements, branches, functions, and lines in separate unit and E2E coverage reports without narrowing the retained release source union. The unit gate's exact 17-source union is the current ten (`release-contract.ts`, `zip-store.ts`, `zip-store-encoder.ts`, `release-assembler.ts`, `release-verifier.ts`, `release-artifact-verifier.ts`, `release-smoke.ts`, `release-private-candidate-reproducibility.ts`, `verify.ts`, `release-coverage.ts`) plus the six new release-set runtime modules and `release-ports.ts`. The E2E gate's exact 13-source union is the current three (`release-assembler.ts`, `release-smoke.ts`, `release-private-candidate-reproducibility.ts`) plus those same six new release-set modules, `release-ports.ts`, `release-coverage.ts`, `release-artifact-verifier.ts`, and `verify.ts`.
...
| `scripts/release/release-private-candidate-reproducibility.ts`                    | Private two-candidate verification, byte comparison, and smoke per application.                                                                                                                                                                                         |
...
| `scripts/release/verify.ts`                                                       | Current public verification entrypoint; Task 7 migrates it to the sole release-set ZIP contract.                                                                                                                                                                        |
...
| `scripts/release/*-test.ts`, `*.e2e.test.ts`, `release-set.e2e.test.ts`           | Current focused unit/E2E contracts plus Task 7's release-set E2E contract, with injected ports and byte fixtures.                                                                                                                                                       |
| `apps/config-search/src/index.ts`, `.test.ts`                                     | Typed unavailable CLI and public process behavior.                                                                                                                                                                                                                      |
| `apps/config-search/package.json`, `apps/bot/package.json`                        | Existing application metadata; excluded from the current 22-path scope.                                                                                                                                                                                                 |
| root `package.json`                                                               | Excluded from the current 22-path scope: existing `release:verify`, `coverage:release`, `coverage:release:unit`, and `coverage:release:e2e` stay unchanged. Only `release:build`, `release:smoke`, and `release:reproducibility` are future separately approved wiring. |
| `scripts/tooling/clean-artifacts.ts`, `.test.ts`                                  | FUTURE separately approved cleaner wiring; excluded from the current 22-path scope.                                                                                                                                                                                     |
| `.github/workflows/ci.yml`, `scripts/tooling/ci-format-workflow-contract.test.ts` | FUTURE separately approved CI/upload wiring; excluded from the current 22-path scope.                                                                                                                                                                                   |
