...
The coordinator MUST proactively decompose work during planning, not only after
an agent runs for a long time. One brief MUST cover one coherent responsibility
and deliverable, its explicit exclusive ownership, and directly coupled
validation only. Separately dispatch discovery or contract work, implementation,
tests or coverage, integration or evidence, final technical review, and final
process review whenever they are independently dispatchable. Unrelated
scenarios, files, or packages MUST NOT be bundled merely to reduce dispatches.

Agent briefs MUST NOT impose arbitrary wall-clock limits or countdown pressure:
these distort work and waste context. The coordinator MUST monitor elapsed time
externally without repeatedly messaging unchanged status. When progress
genuinely stalls, context grows excessively, scope expands, or work becomes
independently separable, the coordinator MUST interrupt or reclassify and
redispatch smaller briefs. A follow-up to the same agent is permitted only for
a narrow finding within unchanged ownership, not for a new task. Parallel work
is permitted only for independent one-writer scopes and MUST preserve the
existing routing and review rules.
